...
static void check_invariants(const char* label)
   ...
   gc_test_walk_young([&](GcHeader* h)
                      { young_set.push_back(h); });
   gc_test_walk_old([&](GcHeader* h)
                    { old_set.push_back(h); });
...
static void force_minor_gc()
   {
   gc_test_force_minor();
   }
static void force_major_gc()
   {
   gc_test_force_major();
   }
...
TEST(leak_type_0_Cons)
   {
   leak_only_test(0);
   }
TEST(leak_type_2_Closure)
   {
   leak_only_test(2);
   }
TEST(leak_type_24_Environment)
   {
   leak_only_test(24);
   }
TEST(leak_type_25_EnvBox)
   {
   leak_only_test(25);
   }
...
TEST(compliance_leak_only_Cons)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(0);
   }
TEST(compliance_leak_only_Closure)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(2);
   }
TEST(compliance_leak_only_Continuation)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(10);
   }
TEST(compliance_leak_only_SyntaxTransformer)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(11);
   }
TEST(compliance_leak_only_Vector)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(12);
   }
TEST(compliance_leak_only_Parameter)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(8);
   }
TEST(compliance_leak_only_RecordMutator)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(19);
   }
TEST(compliance_leak_only_Environment)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(20);
   }
TEST(compliance_leak_only_EnvBox)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(21);
   }
TEST(compliance_leak_only_Integer)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(23);
   }
TEST(compliance_leak_only_Real)
   {
   std::fprintf(stderr, "  [SKIP]\n");
   std::fflush(stderr);
   return;
   compliance_multifile_run(24);
   }
...
struct YoungThresholdGuard
   ...
   ~YoungThresholdGuard()
      {
      gc_test_set_young_threshold(saved);
      }
   };
...
TEST(gc_eval_value_survives_forced_gc)
   ...
   force_minor_gc();
   check_invariants("after minor");
   force_major_gc();
   check_invariants("after major");
...
TEST(gc_cyclic_structure_survives_gc)
   ...
   force_minor_gc();
   check_invariants("after minor on cycle");
   force_major_gc();
   check_invariants("after major on cycle");
