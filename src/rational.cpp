...
Rat Rat::from_float(double f) {
    ...
    if (e >= 0) {
        ...
        int ne = -e;
        if (ne > 62) ne = 62;  // cap denominator to avoid UB (1LL << 63 is UB)
...
Rat::operator double()  const { return static_cast<double>(numerator) / denominator; }
Rat::operator int64_t() const { return numerator / denominator; }  // truncate toward zero
Rat::operator bool()    const { return numerator != 0; }

// ── Unary ─────────────────────────────────────────────────────────────────────

Rat Rat::operator-() const { return Rat(-numerator, denominator); }
...
Rat    Rat::operator+(int64_t n) const { return Rat(numerator + n * denominator, denominator); }
Rat    Rat::operator-(int64_t n) const { return Rat(numerator - n * denominator, denominator); }
Rat    Rat::operator*(int64_t n) const { return Rat(numerator * n, denominator); }
Rat    Rat::operator/(int64_t n) const { return Rat(numerator, denominator * n); }
...
Rat Rat::pow(int exp) const {
    ...
    if (exp > 0) {
        int64_t n = 1, d = 1;
        for (int k = 0; k < exp; ++k) { n *= numerator; d *= denominator; }
        return Rat(n, d);
    }
    // Negative exponent: invert, then raise to positive power
    int pos = -exp;
    int64_t n = 1, d = 1;
    for (int k = 0; k < pos; ++k) { n *= denominator; d *= numerator; }
    return Rat(n, d);
}
...
bool Rat::operator==(int64_t n) const { return denominator == 1 && numerator == n; }
bool Rat::operator!=(int64_t n) const { return !(*this == n); }
bool Rat::operator< (int64_t n) const { return numerator <  n * denominator; }
bool Rat::operator<=(int64_t n) const { return numerator <= n * denominator; }
bool Rat::operator> (int64_t n) const { return numerator >  n * denominator; }
bool Rat::operator>=(int64_t n) const { return numerator >= n * denominator; }
...
Rat    operator+(int64_t n, const Rat& r) { return Rat(n * r.denominator + r.numerator, r.denominator); }
Rat    operator-(int64_t n, const Rat& r) { return Rat(n * r.denominator - r.numerator, r.denominator); }
Rat    operator*(int64_t n, const Rat& r) { return Rat(n * r.numerator, r.denominator); }
Rat    operator/(int64_t n, const Rat& r) { return Rat(n * r.denominator, r.numerator); }
...
int64_t rat_round(const Rat& r) {
    ...
    int64_t twice   = 2 * rem;
    if (twice < r.denominator) return floor_n;
    if (twice > r.denominator) return floor_n + 1;
