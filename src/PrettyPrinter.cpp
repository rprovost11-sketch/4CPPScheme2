...
static bool needs_vertical_bars(const std::string& name) {
    ...
    if (!is_safe_symbol_initial(first) &&
        ...
        return true;
    for (char c : name)
        if (!is_safe_symbol_subsequent(c)) return true;
    return false;
}
...
std::string scheme_pretty_print(const Value& val) {
    if (is_cons(val)) {
        if (scheme_has_cycle(val)) return scheme_pretty_print_shared(val);
    ...
    if (is_record(val)) {
        ...
        if (fs.empty()) return "#<" + tname + '>';
        std::vector<std::string> parts;
        for (const Value& f : fs) parts.push_back(scheme_pretty_print(f));
    ...
    if (is_vector(val)) {
        ...
        std::vector<std::string> parts;
        for (const Value& v : items) parts.push_back(scheme_pretty_print(v));
        return "#(" + join(parts) + ')';
    }
    if (is_bytevector(val)) {
        const std::vector<uint8_t>& items = as_bytevector_items_const(val);
        std::vector<std::string> parts;
        for (uint8_t b : items) parts.push_back(std::to_string(b));
    ...
    if (is_error_object(val)) {
        ...
        std::vector<std::string> parts;
        for (const Value& ir : irs) parts.push_back(scheme_pretty_print(ir));
...
static std::string shared_render(const Value& val, const LabelMap& labels,
    ...
    if (is_cons(val)) {
        ...
        if (lit != labels.end()) {
            int n = lit->second;
            if (seen.count(val_key)) return '#' + std::to_string(n) + '#';
            seen.insert(val_key);
        ...
        while (is_cons(cur)) {
            ...
            if (ck != val_key && labels.count(ck)) {
                tail = cur; found_tail = true; break;
            }
    ...
    if (is_vector(val)) {
        ...
        if (lit != labels.end()) {
            int n = lit->second;
            if (seen.count(key)) return '#' + std::to_string(n) + '#';
            seen.insert(key);
            prefix = '#' + std::to_string(n) + '=';
        }
        std::vector<std::string> parts;
...
std::string scheme_pretty_print_shared(const Value& val) {
    ...
    for (auto& [key, count] : counts)
        if (count > 1) labels[key] = next_label++;
    SeenSet seen;
