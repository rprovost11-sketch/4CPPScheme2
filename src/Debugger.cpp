...
#include <io.h>
static bool _is_tty() { return _isatty(1) != 0; }
#else
#include <unistd.h>
static bool _is_tty() { return isatty(1) != 0; }
#endif
...
void Debugger::_print_scoped_locals(Environment* env, int max_depth) {
    ...
    int scope_num = 0;
    while (current != nullptr && current != global_env) {
        if (max_depth >= 0 && scope_num >= max_depth) break;
        ...
        if (!names_in_scope.empty()) {
            std::sort(names_in_scope.begin(), names_in_scope.end(),
                [](uint32_t a, uint32_t b) { return symbol_name(a) < symbol_name(b); });
...
std::optional<Value> Debugger::_walk_form(const Value& cell,
    ...
    if (!is_cons(cell)) return std::nullopt;
    if (is_symbol(car(cell)) && as_symbol(car(cell)) == callable_name) {
        count++;
        if (count == target_n) return cell;
    }
    Value arg = cdr(cell);
    while (is_cons(arg)) {
...
bool Debugger::_ann_has_any(const Value& cell, const AnnMap& annotations) {
    if (!is_cons(cell)) return false;
    auto* ptr = std::get<ConsCell*>(cell.repr);
    if (annotations.count(ptr)) return true;
    if (_ann_has_any(car(cell), annotations)) return true;
...
std::string Debugger::_ann_flat(const Value& cell, const AnnMap& annotations) {
    ...
    if (is_cons(cell)) {
        ...
        if (it != annotations.end()) tag = it->second;
    }
    if (!is_cons(cell))
...
std::string Debugger::_ann_fmt(const Value& cell, const AnnMap& annotations, int ind) {
    ...
    Value cur = cdr(cell);
    while (is_cons(cur)) {
        if (_ann_has_any(car(cur), annotations)) { has_ann = true; break; }
        cur = cdr(cur);
    }
...
Debugger::_parse_help_entries(const std::string& docstring) {
    ...
    while (std::getline(ss, line)) {
        ...
        for (size_t k = 0; k + 1 < line.size(); k++) {
            if (line[k] == ' ' && line[k + 1] == ' ') { sep = k; break; }
        }
...
void Debugger::_print_help_entries(
    ...
    for (auto& [usage, desc] : entries)
        if (usage.size() > max_w) max_w = usage.size();
    for (auto& [usage, desc] : entries) {
        std::string padded = usage;
        while (padded.size() < max_w) padded += ' ';
...
std::string Debugger::_resolve_bp_ref(const std::string& ref) {
    ...
    for (char c : ref) if (!isdigit((unsigned char)c)) { all_digits = false; break; }
    if (all_digits) {
    ...
    if (breakpoints.count(ref)) return ref;
    for (auto& [ptr, entry] : _inner_targets)
        if (entry.display == ref) return ref;
...
void Debugger::_prune_stale_inner(Environment* env) {
    if (!env || _inner_targets.empty()) return;
    ...
    for (auto& [ptr, entry] : _inner_targets) {
        ...
        if (!same) stale.push_back({ptr, entry.display});
    }
...
void Debugger::_set_inner_breakpoint(const std::string& spec, const std::string& cond,
    ...
    auto parts = [&]() -> std::vector<std::string> {
        ...
        std::string cur;
        for (char c : spec) { if (c == ':') { v.push_back(cur); cur.clear(); } else cur += c; }
        v.push_back(cur);
        return v;
    }();
    ...
    if (parts.size() == 2) {
        fn_name = parts[0]; call_name = parts[1]; index_str = "1";
    } else if (parts.size() == 3) {
        fn_name = parts[0]; call_name = parts[1]; index_str = parts[2];
    } else {
        std::cout << "Invalid breakpoint spec: " << spec << '\n'; return;
    }

    if (!env) { std::cout << "Cannot resolve inner breakpoint without an environment.\n"; return; }

    auto fn_opt = env->lookup_optional(fn_name);
    if (!fn_opt) { std::cout << fn_name << " is not defined.\n"; return; }
    if (!is_closure(*fn_opt)) { std::cout << fn_name << " is not a user-defined procedure.\n"; return; }
    Value fn   = *fn_opt;
    ...
    if (index_str == "?") {
        ...
        try {
            ...
            if (s != std::string::npos) choice = choice.substr(s);
            size_t e = choice.find_last_not_of(" \t\r\n");
            if (e != std::string::npos) choice = choice.substr(0, e + 1);
        } catch (...) {
            std::cout << "\nSet breakpoint cancelled.\n"; return;
        }
        if (choice.empty()) { std::cout << "Set breakpoint cancelled.\n"; return; }
        ...
        for (int n : indices) {
            ...
            _disabled.erase(display);
            if (cond.empty()) std::cout << "Breakpoint set on " << display << ".\n";
            else              std::cout << "Breakpoint set on " << display << " :when " << cond << '\n';
        }
        return;
    }
    ...
    if (idx < 1) { std::cout << "Index must be >= 1.\n"; return; }
...
void Debugger::_cmd_b(const std::string& cmd, const std::string& rest,
    ...
    if (cmd == "b!") {
        if (rest.empty()) { std::cout << "Usage: b! <name-or-number> ...\n"; return; }
    ...
    if (cmd == "b") {
        if (rest.empty() || rest == "*") {
            ...
            for (auto& [n, c] : breakpoints) bp_names.push_back(n);
            ...
            if (!has_any) std::cout << "No breakpoints set.\n";
            return;
        }
        ...
        for (auto& [name, cond] : pairs) {
            if (name.find(':') != std::string::npos) {
                ...
                _disabled.erase(name);
                if (cond.empty()) std::cout << "Breakpoint set on " << name << ".\n";
                else              std::cout << "Breakpoint set on " << name << " :when " << cond << '\n';
            }
        }
        return;
    }
    ...
    // cmd == "b-"
    if (rest.empty()) { std::cout << "Usage: b- <name-or-number> ... or b- *\n"; return; }
    if (rest == "*") {
        breakpoints.clear(); _inner_targets.clear();
        _disabled.clear();   _bp_index.clear();
    ...
    while (ss >> ref) {
        if (ref.find(':') != std::string::npos && ref.size() >= 2 &&
            ...
            std::string cur;
            for (char c : ref) { if (c == ':') { pparts.push_back(cur); cur.clear(); } else cur += c; }
            ...
            if (pparts.size() == 2) {
                ...
                for (auto it = _inner_targets.begin(); it != _inner_targets.end(); ) {
                    if (it->second.fn_name == pparts[0]) {
                        ...
                        it = _inner_targets.erase(it); count++;
                    } else ++it;
                }
            ...
            if (count) std::cout << count << " breakpoint" << (count > 1 ? "s" : "") << " removed.\n";
            else       std::cout << "No matching breakpoints.\n";
            continue;
        }
        std::string name = _resolve_bp_ref(ref);
        if (name.empty()) continue;
        if (breakpoints.count(name)) {
            ...
            if (!removed) std::cout << "No breakpoint on " << name << ".\n";
        }
    }
}
...
void Debugger::_cmd_bc(const std::string& cmd, const std::string& rest,
    ...
    if (cmd == "bc-") {
        ...
        while (ss >> ref) {
            ...
            if (breakpoints.count(name)) {
                ...
                if (!found) std::cout << "No breakpoint on " << name << ".\n";
            }
        }
        return;
    }
    ...
    for (auto& [ref, cond] : pairs) {
        if (cond.empty()) { std::cout << "Missing condition for " << ref << ".\n"; continue; }
        std::string name = _resolve_bp_ref(ref);
        if (name.empty()) continue;
        if (breakpoints.count(name)) {
            breakpoints[name] = cond;
...
void Debugger::_cmd_bo(const std::string& cmd, const std::string& rest,
    ...
    if (cmd == "bo+") {
        ...
        std::sort(bo.begin(), bo.end());
        std::string joined;
        for (size_t k = 0; k < bo.size(); k++) { if (k) joined += ", "; joined += bo[k]; }
        std::cout << "Break-on restored: " << joined << '\n';
    } else if (cmd == "bo-") {
        if (rest.empty()) { std::cout << "Usage: bo- <name> or bo- *\n"; return; }
        if (rest == "*") { break_on.clear(); std::cout << "Break-on list cleared.\n"; return; }
        std::istringstream ss(rest);
        std::string n;
        while (ss >> n) break_on.erase(n);
        if (!break_on.empty()) {
            std::vector<std::string> bo(break_on.begin(), break_on.end());
            std::sort(bo.begin(), bo.end());
            std::string joined;
...
void Debugger::_cmd_bt(const std::string&, const std::string&,
    ...
    if (stack.empty()) { std::cout << "No backtrace available.\n"; return; }
...
void Debugger::_cmd_body(const std::string&, const std::string& rest,
    ...
    if (rest.empty()) { std::cout << "Usage: body <fn>\n"; return; }
    std::string fn_name = rest;
    {
        size_t s = fn_name.find_first_not_of(" \t");
        if (s != std::string::npos) fn_name = fn_name.substr(s);
        size_t e = fn_name.find_last_not_of(" \t");
        if (e != std::string::npos) fn_name = fn_name.substr(0, e + 1);
    }
    auto fn_opt = env->lookup_optional(fn_name);
    if (!fn_opt) { std::cout << fn_name << " is not defined.\n"; return; }
    if (!is_closure(*fn_opt)) { std::cout << fn_name << " is not a user-defined procedure.\n"; return; }
...
void Debugger::_cmd_e(const std::string&, const std::string& rest,
                       Context* ctx, Environment* env) {
    if (!rest.empty()) safe_eval(ctx, env, rest);
}
...
void Debugger::_cmd_h(const std::string&, const std::string& rest,
    ...
    if (!rest.empty()) {
        ...
        if (s != std::string::npos) base = base.substr(s);
        ...
        if (it != _cmd_help.end()) {
            ...
            if (!entries.empty()) _print_help_entries(entries);
    ...
    for (const auto& base : _help_order) {
        auto it = _cmd_help.find(base);
        if (it == _cmd_help.end()) continue;
        auto these = _parse_help_entries(it->second);
        for (auto& e2 : these) all_entries.push_back(e2);
    }
...
void Debugger::_cmd_i(const std::string&, const std::string& rest,
                       Context* ctx, Environment* env) {
    if (!rest.empty()) safe_inspect(ctx, env, rest);
}
...
void Debugger::_cmd_rd(const std::string&, const std::string& rest,
    ...
    std::string expr;
    if (rest.empty()) {
        if (_last_rd_expr.empty()) { std::cout << "No previous rd expression.\n"; return; }
        expr = _last_rd_expr;
    } else {
        expr = rest;
        _last_rd_expr = expr;
...
void Debugger::_cmd_v(const std::string&, const std::string& rest,
    ...
    if (rest.empty()) {
        ...
        for (char c : rest) if (!isdigit((unsigned char)c)) { all_digits = false; break; }
...
void Debugger::_cmd_w(const std::string& cmd, const std::string& rest,
                       Context* ctx, Environment* env) {
    if (cmd == "w-") {
        if (rest.empty()) { std::cout << "Usage: w- <expr-or-number> or w- *\n"; return; }
        if (rest == "*") { watch_list.clear(); std::cout << "Watch list cleared.\n"; return; }
        bool all_digits = true;
        for (char c : rest) if (!isdigit((unsigned char)c)) { all_digits = false; break; }
        if (all_digits && !rest.empty()) {
            ...
            for (auto& canonical : _parse_watch_args(rest)) {
                ...
                if (it != watch_list.end()) watch_list.erase(it);
                else std::cout << canonical << " not in watch list.\n";
            }
            if (!watch_list.empty()) {
    ...
    if (rest.empty() || rest == "*") {
        ...
        std::string joined;
        for (size_t k = 0; k < watch_list.size(); k++) { if (k) joined += ", "; joined += watch_list[k]; }
    ...
    for (const auto& entry : watch_list) {
        try {
            ...
            for (auto& form : forms) {
                ...
                analyze(expanded);
                if (ctx->lEval) result = ctx->lEval(env, expanded);
            }
    ...
    try {
        while (true) {
            ...
            try {
                ...
                std::cout << '\n';
                if (in_execution) { _swap_history_out(); return "abort"; }
                continue;
            }
#else
            ...
            if (line.empty()) {
                if (in_execution) { _swap_history_out(); return "step"; }
                continue;
            }
            ...
            if (line == "s") {
                if (!in_execution) { std::cout << "Not in execution.  Use rd to run.\n"; continue; }
                if (!_step_hook) _step_hook = StepHook{};
                _swap_history_out();
                return "step";
            }
            if (line == "n") {
                ...
                if (step_hook) {
                    ...
                    StepHook hook; hook._skip_until_depth = depth;
                    _step_hook = hook;
                }
                _swap_history_out();
                return "step";
            ...
            if (line == "o") {
                if (!in_execution) { std::cout << "Not in execution.  Use rd to run.\n"; continue; }
                if (depth == 0) { std::cout << "Already at top level.\n"; continue; }
                if (step_hook) {
                    step_hook->_skip_until_depth = depth - 1;
                } else {
                    StepHook hook; hook._skip_until_depth = depth - 1;
                    _step_hook = hook;
                }
                _swap_history_out();
                return "step";
            ...
            if (line == "abort") {
                if (!in_execution) { std::cout << "Not in execution.\n"; continue; }
                _swap_history_out();
                return "abort";
            }
            ...
            try {
                ...
                if (!handled) std::cout << "Unknown command.  Type h for help.\n";
            } catch (_RestartRd&) {
                _swap_history_out();
                throw;
...
void Debugger::on_expr(const Value& C, Environment* E, const KStack& K, Context* ctx) {
    ...
    if (bp_fired && !cond_src.empty()) {
        ...
        try {
            ...
            for (auto& form : forms) {
                Value expanded = expand(form);
                analyze(expanded);
                if (ctx->lEval) result = ctx->lEval(E, expanded);
            }
            bool is_false_val = is_boolean(result) && !as_boolean(result);
            if (is_false_val || is_nil(result)) bp_fired = false;
        } catch (...) {
            bp_fired = false;
        }
        _step_hook = saved_hook;
    ...
    // ── Step check ──
    if (!_step_hook) return;
    StepHook& sh = *_step_hook;
    int depth = (int)K.size();

    if (sh._skip_until_depth.has_value()) {
        if (depth > *sh._skip_until_depth) return;
...
Value Debugger::run_debugger_repl(Context* ctx, Environment* env) {
    ...
    if (!breakpoints.empty()) {
        ...
        std::vector<std::string> names;
        for (auto& [n, c] : breakpoints) names.push_back(n);
        std::sort(names.begin(), names.end());
        for (const auto& name : names) {
            const auto& cond = breakpoints.at(name);
            if (cond.empty()) std::cout << "  " << name << '\n';
            else              std::cout << "  " << name << "  :when " << cond << '\n';
        }
    }
    if (!watch_list.empty()) {
        std::string joined;
