...
void set_runtime_env(Environment* env)
   ...
   if (!s_registered)
      {
      gc_env_root_push(&g_runtime_env);
      s_registered = true;
      }
   g_runtime_env = env;
   }
...
void set_include_fallback_dir(const std::string& dir)
   {
   g_include_fallback_dir = dir;
   }
std::string get_include_fallback_dir()
   {
...
static Value rename_refs_in_form(const Value& form, const RenameTable& table)
   ...
   if (is_symbol(head))
      ...
      if (hid == sid_lambda)
         return rrif_lambda(form, table);
      if (hid == sid_let || hid == sid_let_star ||
          hid == sid_letrec || hid == sid_letrec_star)
         return rrif_let(form, table, hid);
      if (hid == sid_case_lambda)
         return rrif_case_lambda(form, table);
      if (hid == intern_symbol("case"))
         return rrif_case(form, table);
...
static Value rrif_lambda(const Value& form, const RenameTable& table)
   {
   if (!is_cons(cdr(form)))
      return rrif_default(form, table);
...
static Value expand_body(const Value& body_cons)
   ...
   while (i < forms.size())
      ...
      if (is_head(f, sid_define_values))
         {
         if (!is_cons(cdr(f)) || !is_cons(cdr(cdr(f))) || !is_nil(cdr(cdr(cdr(f)))))
            break;
         ...
         if (has_rest)
            ...
            Value tmp_rest_sym = has_rest ? make_symbol(hygiene_gensym("mv-tmp-rest"), fsrc) : NIL_VALUE;
            ...
            for (Value& v : body_items)
               consumer_items.push_back(v);
   ...
   for (Value& v : body_prefix)
      rest_forms.push_back(std::move(v));
   for (; i < forms.size(); i++)
      rest_forms.push_back(forms[i]);
...
static Value qq_walk(const Value& x, int level, SourceInfo* /*default_src*/)
   ...
   if (is_cons(x))
      ...
      if (is_cons(head) && is_symbol(car(head)) &&
         ...
         if (level == 1)
            return qq_make_append(expand(e), tail);
...
bool feature_req_matches(const Value& req)
   ...
   if (op == sid_and_kw)
      ...
      while (is_cons(cur))
         {
         if (!feature_req_matches(car(cur)))
            return false;
         cur = cdr(cur);
         }
      return true;
...
static Value expand_define(const Value& sexpr)
   ...
   if (!is_cons(signature) || is_nil(body_cons))
      return expand_list(sexpr);
...
static Value expand_let(const Value& s)
   {
   return expand_let_family(s, "let");
   }
static Value expand_let_star(const Value& s)
   {
   return expand_let_family(s, "let*");
   }
static Value expand_letrec(const Value& s)
   {
   return expand_let_family(s, "letrec");
   }
static Value expand_letrec_star(const Value& s)
   {
   return expand_let_family(s, "letrec*");
   }
...
static Value expand_when(const Value& s)
   {
   return expand_when_unless(s, "when");
   }
static Value expand_unless(const Value& s)
   {
   return expand_when_unless(s, "unless");
   }
...
static Value expand_do(const Value& sexpr)
   ...
   if (list_length(bindings_cons) < 0)
      return expand_list(sexpr);
   if (!is_cons(test_cons) || list_length(test_cons) < 1)
      return expand_list(sexpr);
   ...
   while (is_cons(cur))
      ...
      if (seen_vars.count(vname))
         return expand_list(sexpr);
      seen_vars.insert(vname);
      vars_list.push_back(car(b));
      inits_list.push_back(expand(car(cdr(b))));
      if (blen == 3)
         steps_list.push_back(expand(car(cdr(cdr(b)))));
      else
         steps_list.push_back(NIL_VALUE); // sentinel: use var itself
      cur = cdr(cur);
      }
   ...
   for (size_t i = 0; i < vars_list.size(); i++)
      {
      if (!is_nil(steps_list[i]))
         call_items.push_back(steps_list[i]);
      else
         call_items.push_back(vars_list[i]);
      }
   ...
   while (is_cons(cur))
      {
      body_items.push_back(expand(car(cur)));
      cur = cdr(cur);
      }
   Value body_form;
   if (body_items.empty())
   ...
   while (is_cons(cur))
      {
      result_items.push_back(expand(car(cur)));
      cur = cdr(cur);
      }
   Value result_form;
...
static Value expand_include_form(const Value& sexpr, bool fold)
   ...
   while (is_cons(cur))
      ...
      for (Value& v : from_file)
         all_forms.push_back(std::move(v));
      cur = cdr(cur);
      }
   if (all_forms.empty())
      return expand_list(sexpr);
   std::vector<Value> items = {make_symbol("begin", nullptr)};
   for (Value& v : all_forms)
      items.push_back(std::move(v));
   return list_from_items(items, nullptr);
   }
static Value expand_include(const Value& s)
   {
   return expand_include_form(s, false);
   }
static Value expand_include_ci(const Value& s)
   {
   return expand_include_form(s, true);
   }
...
static Value expand_cond_expand(const Value& sexpr)
   ...
   while (is_cons(cur))
      ...
      if (feature_req_matches(req))
         ...
         while (is_cons(bcur))
            {
            items.push_back(expand(car(bcur)));
            bcur = cdr(bcur);
            }
         return list_from_items(items, nullptr);
         }
      ...
      if (mvf.has_rest)
         inner_binding_pairs.push_back(list_from_items(
             {make_symbol(mvf.rest, nullptr),
              mv_tail_ref(tmp, (int)mvf.fixed.size())},
             nullptr));
      }
...
static Value expand_guard(const Value& sexpr)
   ...
   Value body_cons = cdr(cdr(sexpr));
   if (list_length(guard_spec) < 1)
      return expand_list(sexpr);
   Value var_sexpr = car(guard_spec);
   if (!is_symbol(var_sexpr))
      return expand_list(sexpr);
   Value clauses_cons = cdr(guard_spec);
   if (list_length(clauses_cons) < 0)
      return expand_list(sexpr);
   ...
   while (is_cons(cur))
      {
      cond_items.push_back(expand_list(car(cur)));
      cur = cdr(cur);
      }
   if (!has_else)
      {
...
static Value expand_define_record_type(const Value& sexpr)
   ...
   if (!is_symbol(type_name_sexpr) || !is_symbol(pred_sexpr))
      return expand_list(sexpr);
   if (list_length(ctor_spec) < 1)
      return expand_list(sexpr);
   Value ctor_name = car(ctor_spec);
   if (!is_symbol(ctor_name))
      return expand_list(sexpr);
   ...
   while (is_cons(cur))
      ...
      if (!is_symbol(fname) || !is_symbol(accessor))
         return expand_list(sexpr);
      if (slen == 3)
         {
         Value mutator = car(cdr(cdr(spec)));
         if (!is_symbol(mutator))
            return expand_list(sexpr);
   ...
   for (const auto& fe : field_entries)
      all_field_names.push_back(fe.fname);

   std::string rt_sym_name = hygiene_gensym("record-type-" + type_name);
   SourceInfo* src = src_of(sexpr);
   Value rt_sym = make_symbol(rt_sym_name, src);

   auto mk_sym = [src](const std::string& n)
   ...
   for (const Value& fn : all_field_names)
      {
      if (ctor_field_names_set.count(as_symbol(fn)))
         list_items.push_back(fn);
      else
         list_items.push_back(VOID_VALUE);
      }
   ...
   for (int i = 0; i < (int)field_entries.size(); i++)
      ...
      Value acc_init = list_from_items(
          {mk_sym("%make-record-accessor"), rt_sym, idx_val,
           mk_quote(field_entries[i].accessor)},
          nullptr);
      ...
      if (field_entries[i].has_mutator)
         ...
         Value mut_init = list_from_items(
             {mk_sym("%make-record-mutator"), rt_sym, idx_val,
              mk_quote(field_entries[i].mutator)},
             nullptr);
...
static Value expand_inner(Value sexpr)
   ...
   for (int iterations = 0;; iterations++)
      ...
      if (is_symbol(head))
         ...
         if (name_id == sid_define_syntax)
            return expand_define_syntax(sexpr);
         if (name_id == sid_let_syntax)
            return expand_let_syntax(sexpr, false);
         if (name_id == sid_letrec_syntax)
            return expand_let_syntax(sexpr, true);
         // User macro
