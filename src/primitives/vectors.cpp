...
static Value _prim_vector_to_list(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 2) {
        if (!is_integer(args[1])) throw SchemeTypeError("vector->list: start must be an integer", _src(app));
        start = as_integer(args[1]);
    }
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("vector->list: end must be an integer", _src(app));
...
static Value _prim_vector_fill(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    int64_t start = 0, end = static_cast<int64_t>(items.size());
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("vector-fill!: start must be an integer", _src(app));
        start = as_integer(args[2]);
    }
    if (args.size() >= 4) {
        if (!is_integer(args[3])) throw SchemeTypeError("vector-fill!: end must be an integer", _src(app));
...
static Value _prim_vector_copy(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    int64_t start = 0, end = static_cast<int64_t>(items.size());
    if (args.size() >= 2) {
        if (!is_integer(args[1])) throw SchemeTypeError("vector-copy: start must be an integer", _src(app));
        start = as_integer(args[1]);
    }
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("vector-copy: end must be an integer", _src(app));
        end = as_integer(args[2]);
    }
    if (start < 0 || end > static_cast<int64_t>(items.size()) || start > end)
...
static Value _prim_vector_copy_bang(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 4) {
        if (!is_integer(args[3])) throw SchemeTypeError("vector-copy!: start must be an integer", _src(app));
        start = as_integer(args[3]);
    }
    if (args.size() >= 5) {
        if (!is_integer(args[4])) throw SchemeTypeError("vector-copy!: end must be an integer", _src(app));
...
static Value _prim_vector_to_string(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    int64_t start = 0, end = static_cast<int64_t>(items.size());
    if (args.size() >= 2) {
        if (!is_integer(args[1])) throw SchemeTypeError("vector->string: start must be an integer", _src(app));
        start = as_integer(args[1]);
    }
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("vector->string: end must be an integer", _src(app));
        end = as_integer(args[2]);
    }
    if (start < 0 || end > static_cast<int64_t>(items.size()) || start > end)
...
static Value _prim_string_to_vector(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 2) {
        if (!is_integer(args[1])) throw SchemeTypeError("string->vector: start must be an integer", _src(app));
        start = as_integer(args[1]);
    }
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("string->vector: end must be an integer", _src(app));
        end = as_integer(args[2]);
    }
...
static Value _prim_vector_for_each(Context* ctx, Environment* env, std::vector<Value>& args, const Value* app) {
    ...
    for (size_t vi = 1; vi < vecs.size(); ++vi)
        if (vecs[vi]->size() < shortest) shortest = vecs[vi]->size();
    for (size_t i = 0; i < shortest; ++i) {
        std::vector<Value> row;
        GcRootVec row_root(row);
        for (auto* v : vecs) row.push_back((*v)[i]);
