...
static Value _prim_real_p(Context*, Environment*, std::vector<Value>& args, const Value*) {
    ...
    if (is_integer(v) || is_bignum(v) || is_real(v) || is_rational(v)) return make_boolean(true);
    if (is_complex(v)) return make_boolean(as_complex_imag(v) == 0.0);
...
static Value _prim_rational_p(Context*, Environment*, std::vector<Value>& args, const Value*) {
    const Value& v = args[0];
    if (is_integer(v) || is_bignum(v) || is_rational(v)) return make_boolean(true);
    if (is_real(v)) return make_boolean(std::isfinite(as_real(v)));
    if (is_complex(v))
...
static Value _prim_integer_p(Context*, Environment*, std::vector<Value>& args, const Value*) {
    const Value& v = args[0];
    if (is_integer(v) || is_bignum(v)) return make_boolean(true);
    if (is_real(v)) {
        double r = as_real(v);
    ...
    if (is_exact_complex(v)) {
        Value im = as_exact_complex_imag(v);
        if (!is_integer(im) || as_integer(im) != 0) return make_boolean(false);
...
static double _real_or_error(const Value& v, const char* name, const Value* app) {
    ...
    if (is_exact_complex(v)) {
        ...
        if (is_integer(im) && as_integer(im) == 0) {
            ...
            if (is_integer(re))  return static_cast<double>(as_integer(re));
            if (is_rational(re)) return static_cast<double>(Rat{as_rational_num(re), as_rational_den(re)});
        }
    }
...
static int64_t _int_or_error(const Value& v, const char* name, const Value* app) {
    ...
    if (is_real(v)) {
        double r = as_real(v);
        if (std::isfinite(r) && std::floor(r) == r) return static_cast<int64_t>(r);
    }
...
static Value _prim_even_p(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_bignum(v)) return make_boolean(mpz_divisible_ui_p(as_bignum(v), 2) != 0);
...
static Value _prim_finite_p(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_integer(v) || is_bignum(v) || is_rational(v) || is_exact_complex(v)) return make_boolean(true);
    if (is_real(v))    return make_boolean(std::isfinite(as_real(v)));
    if (is_complex(v)) return make_boolean(std::isfinite(as_complex_real(v)) && std::isfinite(as_complex_imag(v)));
...
static Value _prim_infinite_p(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_integer(v) || is_bignum(v) || is_rational(v) || is_exact_complex(v)) return make_boolean(false);
    if (is_real(v))    return make_boolean(std::isinf(as_real(v)));
    if (is_complex(v)) return make_boolean(std::isinf(as_complex_real(v)) || std::isinf(as_complex_imag(v)));
...
static Value _prim_nan_p(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_integer(v) || is_bignum(v) || is_rational(v) || is_exact_complex(v)) return make_boolean(false);
    if (is_real(v))    return make_boolean(std::isnan(as_real(v)));
    if (is_complex(v)) return make_boolean(std::isnan(as_complex_real(v)) || std::isnan(as_complex_imag(v)));
...
void register_predicates() {
    ...
    register_primitive("integer?",  1, 1, _prim_integer_p,  "",
        ...
        "(like 3.0) is also considered an integer.", CATEGORY);
