...
static Value _string_compare(const char* name, std::vector<Value>& args, const Value* app,
    ...
    if (args.size() < 2) return make_boolean(true);
    ...
    for (size_t i = 1; i < args.size(); ++i) {
        ...
        if (!op(*prev, cur)) return make_boolean(false);
        prev = &cur;
    }
    return make_boolean(true);
}

static Value _prim_string_eq(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare("string=?",  a, n, [](const std::string& x, const std::string& y){ return x == y; }); }
static Value _prim_string_lt(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare("string<?",  a, n, [](const std::string& x, const std::string& y){ return x <  y; }); }
static Value _prim_string_le(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare("string<=?", a, n, [](const std::string& x, const std::string& y){ return x <= y; }); }
static Value _prim_string_gt(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare("string>?",  a, n, [](const std::string& x, const std::string& y){ return x >  y; }); }
static Value _prim_string_ge(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare("string>=?", a, n, [](const std::string& x, const std::string& y){ return x >= y; }); }
...
static Value _prim_substring(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (!is_integer(args[1])) throw SchemeTypeError("substring: start must be an integer", _src(app));
    int64_t nchars = utf8_char_count(s);
    int64_t start = as_integer(args[1]);
    int64_t end   = nchars;
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("substring: end must be an integer", _src(app));
...
static Value _prim_string_to_list(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 2) {
        if (!is_integer(args[1])) throw SchemeTypeError("string->list: start must be an integer", _src(app));
        start = as_integer(args[1]);
    }
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("string->list: end must be an integer", _src(app));
        end = as_integer(args[2]);
    }
    if (start < 0 || end > nchars || start > end)
...
static Value _prim_string_copy(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    int64_t start = 0, end = nchars;
    if (args.size() >= 2) {
        if (!is_integer(args[1])) throw SchemeTypeError("string-copy: start must be an integer", _src(app));
        start = as_integer(args[1]);
    }
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("string-copy: end must be an integer", _src(app));
        end = as_integer(args[2]);
    }
    if (start < 0 || end > nchars || start > end)
...
static Value _prim_make_string(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (k < 0) throw SchemeTypeError("make-string: length must be non-negative", _src(app));
    char32_t fill = ' ';
    if (args.size() >= 2) {
        if (!is_character(args[1])) throw SchemeTypeError("make-string: fill must be a character", _src(app));
        fill = as_character(args[1]);
    }
    std::string unit;
    utf8_encode(unit, fill);
    std::string result;
    for (int64_t i = 0; i < k; ++i) result += unit;
    return make_string(result);
}
...
static Value _prim_string_upcase(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    for (char& c : s) c = static_cast<char>(std::toupper(static_cast<uint8_t>(c)));
...
static Value _prim_string_downcase(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    for (char& c : s) c = static_cast<char>(std::tolower(static_cast<uint8_t>(c)));
    return make_string(s);
}
...
static std::string _to_lower(const std::string& s) {
    std::string r = s;
    for (char& c : r) c = static_cast<char>(std::tolower(static_cast<uint8_t>(c)));
    return r;
}
...
static Value _prim_string_ci_eq(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare_ci("string-ci=?",  a, n, [](const std::string& x, const std::string& y){ return x == y; }); }
static Value _prim_string_ci_lt(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare_ci("string-ci<?",  a, n, [](const std::string& x, const std::string& y){ return x <  y; }); }
static Value _prim_string_ci_le(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare_ci("string-ci<=?", a, n, [](const std::string& x, const std::string& y){ return x <= y; }); }
static Value _prim_string_ci_gt(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare_ci("string-ci>?",  a, n, [](const std::string& x, const std::string& y){ return x >  y; }); }
static Value _prim_string_ci_ge(Context*, Environment*, std::vector<Value>& a, const Value* n)  { return _string_compare_ci("string-ci>=?", a, n, [](const std::string& x, const std::string& y){ return x >= y; }); }
...
static Value _prim_string_map(Context* ctx, Environment* env, std::vector<Value>& args, const Value* app) {
    ...
    for (;;) {
        ...
        for (size_t si = 0; si < strs.size(); ++si)
            if (positions[si] >= strs[si]->size()) { any_done = true; break; }
        if (any_done) break;
...
static Value _prim_string_set_bang(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (k < 0 || static_cast<size_t>(k) >= sm.size())
        throw SchemeTypeError(
            ...
            " out of range for string of length " + std::to_string(sm.size()), _src(app));
...
static Value _prim_string_fill_bang(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("string-fill!: start must be an integer", _src(app));
        start = as_integer(args[2]);
    }
    if (args.size() >= 4) {
        if (!is_integer(args[3])) throw SchemeTypeError("string-fill!: end must be an integer", _src(app));
...
static Value _prim_string_copy_bang(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 4) {
        if (!is_integer(args[3])) throw SchemeTypeError("string-copy!: start must be an integer", _src(app));
        start = as_integer(args[3]);
    }
    if (args.size() >= 5) {
        if (!is_integer(args[4])) throw SchemeTypeError("string-copy!: end must be an integer", _src(app));
