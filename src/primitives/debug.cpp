...
static std::string _scheme_type_name(const Value& val)
   ...
   if (is_record(val))
      return "record";
   if (is_record_accessor(val))
      return "record-accessor";
   if (is_record_mutator(val))
...
static void _describe_object(const Value& val, Context* ctx)
   {
   auto wl = [&](const std::string& line)
   { ctx->write(line + "\n"); };

   if (is_nil(val))
      {
      wl("() is the empty list (null)");
      return;
      }
   if (is_void(val))
      {
   ...
   if (is_boolean(val))
      {
      wl(std::string(as_boolean(val) ? "#t" : "#f") + " is a boolean");
      return;
      }
   if (is_integer(val))
      {
      wl(std::to_string(as_integer(val)) + " is an integer");
      return;
      }
   if (is_real(val))
      {
      wl(scheme_pretty_print(val) + " is a real");
      return;
      }
   if (is_rational(val))
      {
   ...
   if (is_character(val))
      {
      wl(scheme_pretty_print(val) + " is a character");
      return;
      }
   if (is_symbol(val))
      {
      wl(as_symbol(val) + " is a symbol");
      return;
      }
   ...
   if (is_primitive(val))
      ...
      if (it != ph.end())
         {
         if (it->second.kind == "special")
            wl("  Kind: special form");
   ...
   if (is_continuation(val))
      {
      wl("#<continuation> is a continuation");
      return;
      }
   if (is_multi_values(val))
      {
...
static std::vector<InspectChild> _inspectable_children(const Value& val)
   ...
   if (is_cons(val))
      ...
      while (is_cons(cur))
         {
         result.push_back({std::to_string(idx++), car(cur)});
         cur = cdr(cur);
         }
      if (!is_nil(cur))
         result.push_back({"tail", cur});
      return result;
      }
   if (is_multi_values(val))
      {
