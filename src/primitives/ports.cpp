...
static void _register_port_roots_once() {
    if (s_port_roots_registered) return;
...
static Value _stdin_port()  { return _make_stdio_port(true,  true,  stdin,  "<stdin>"); }
static Value _stdout_port() { return _make_stdio_port(false, true,  stdout, "<stdout>"); }
static Value _stderr_port() { return _make_stdio_port(false, true,  stderr, "<stderr>"); }
...
static std::string _render_display(const Value& val) {
    ...
    if (is_character(val)) return _char32_to_utf8(as_character(val));
    if (is_void(val))      return "";
    if (is_cons(val)) {
...
static size_t _skip_one_expr(const std::vector<Token>& toks, size_t i) {
    ...
    if (k == TokenKind::LPAREN || k == TokenKind::LBRACKET ||
        ...
        while (i < toks.size() && toks[i].kind != close && toks[i].kind != TokenKind::END_OF_FILE) {
            if (toks[i].kind == TokenKind::DOT) { i++; continue; }
            i = _skip_one_expr(toks, i);
        }
        if (i < toks.size() && toks[i].kind == close) i++; // skip closer
        return i;
    }
...
static Value _prim_input_port_open_p(Context*, Environment*, std::vector<Value>& args, const Value*) {
    if (!is_port(args[0])) return make_boolean(false);
...
static Value _prim_open_binary_input_file(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (sz > 0) std::fread(data.data(), 1, static_cast<size_t>(sz), fh);
...
static Value _prim_read(Context* ctx, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (p->pos >= full.size()) return make_eof();
    ...
    for (size_t i = p->pos; i < full.size(); ++i) {
        ...
        if (c > ' ') { has_nonws = true; break; }
    }
    if (!has_nonws) return make_eof();
...
static Value _prim_read_string(Context* ctx, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (p->pos >= p->buf_text.size() && k > 0) return make_eof();
...
static Value _prim_read_u8(Context* ctx, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (p->pos >= p->buf_binary.size()) return make_eof();
...
static Value _prim_read_bytevector(Context* ctx, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (p->pos >= p->buf_binary.size() && k > 0) return make_eof();
...
static Value _prim_read_bytevector_bang(Context* ctx, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("read-bytevector!: start must be an integer", _src(app));
        start = as_integer(args[2]);
    }
    if (args.size() >= 4) {
        if (!is_integer(args[3])) throw SchemeTypeError("read-bytevector!: end must be an integer", _src(app));
    ...
    if (p->pos >= p->buf_binary.size() && want > 0) return make_eof();
...
static Value _prim_write_string(Context* ctx, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("write-string: start must be an integer", _src(app));
        start = as_integer(args[2]);
    }
    if (args.size() >= 4) {
        if (!is_integer(args[3])) throw SchemeTypeError("write-string: end must be an integer", _src(app));
        end = as_integer(args[3]);
    }
...
static Value _prim_write_bytevector(Context* ctx, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (args.size() >= 3) {
        if (!is_integer(args[2])) throw SchemeTypeError("write-bytevector: start must be an integer", _src(app));
        start = as_integer(args[2]);
    }
    if (args.size() >= 4) {
        if (!is_integer(args[3])) throw SchemeTypeError("write-bytevector: end must be an integer", _src(app));
        end = as_integer(args[3]);
    }
...
static Value _prim_flush_output_port(Context* ctx, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (p->file_h) std::fflush(p->file_h);
    return VOID_VALUE;
}
...
static Value _prim_delete_file(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (ec) throw SchemeFileError("delete-file: " + ec.message(), _src(app));
    if (!removed) throw SchemeFileError("delete-file: file not found: " + path, _src(app));
    return VOID_VALUE;
}
...
static Value _prim_rename_file(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (ec) throw SchemeFileError("rename-file: " + ec.message(), _src(app));
    return VOID_VALUE;
}
