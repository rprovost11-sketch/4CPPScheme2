...
static bool _eqv_pred(const Value& a, const Value& b)
   {
   return eqv_atom(a, b);
   }

static bool _equal_pred(const Value& a, const Value& b)
   {
...
static Value _prim_list(Context*, Environment*, std::vector<Value>& args, const Value*)
   ...
   while (i >= 0)
      {
      result = alloc_cons(args[i], result);
      --i;
      }
   return result;
   }
...
static Value _prim_reverse(Context*, Environment*, std::vector<Value>& args, const Value* app)
   ...
   while (is_cons(cur))
      {
      result = alloc_cons(car(cur), result);
      cur = cdr(cur);
      }
   if (!is_nil(cur))
...
static Value _prim_list_copy(Context*, Environment*, std::vector<Value>& args, const Value*)
   ...
   while (i >= 0)
      {
      tail = alloc_cons(collected[i], tail);
      --i;
      }
   return tail;
   }
...
static Value _member_search(const char* name, _EqFn eq_pred,
   ...
   if (args.size() >= 3)
      ...
      while (is_cons(cur.val))
         {
         if (eq_pred(target.val, car(cur.val)))
            return cur.val;
         cur.val = cdr(cur.val);
         }
      }
...
static Value _assoc_search(const char* name, _EqFn eq_pred,
   ...
   if (args.size() >= 3)
      ...
      while (is_cons(cur.val))
         ...
         if (!is_cons(pair))
            throw SchemeTypeError(std::string(name) + ": alist entries must be pairs", _src(app));
         if (eq_pred(target.val, car(pair)))
            return pair;
         cur.val = cdr(cur.val);
         }
      }
...
static Value _prim_member(Context* c, Environment*, std::vector<Value>& a, const Value* n)
   {
   return _member_search("member", _equal_pred, c, a, n);
   }
static Value _prim_memv(Context* c, Environment*, std::vector<Value>& a, const Value* n)
   {
   return _member_search("memv", _eqv_pred, c, a, n);
   }
static Value _prim_memq(Context* c, Environment*, std::vector<Value>& a, const Value* n)
   {
   return _member_search("memq", _eq_pred, c, a, n);
   }
static Value _prim_assoc(Context* c, Environment*, std::vector<Value>& a, const Value* n)
   {
   return _assoc_search("assoc", _equal_pred, c, a, n);
   }
static Value _prim_assv(Context* c, Environment*, std::vector<Value>& a, const Value* n)
   {
   return _assoc_search("assv", _eqv_pred, c, a, n);
   }
static Value _prim_assq(Context* c, Environment*, std::vector<Value>& a, const Value* n)
   {
   return _assoc_search("assq", _eq_pred, c, a, n);
   }
...
static Value _prim_map(Context* ctx, Environment*, std::vector<Value>& args, const Value* app)
   ...
   while (i >= 0)
      {
      result = alloc_cons(collected[i], result);
      --i;
      }
   return result;
   }
