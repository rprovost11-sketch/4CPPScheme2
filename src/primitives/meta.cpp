...
static Value _prim_record_ref(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (!is_record(args[0]) || as_record_type(args[0]) != rt)
        throw SchemeTypeError(
            ...
            as_record_type_name(args[1]), _src(app));
...
static Value _prim_exit(Context*, Environment*, std::vector<Value>& args, const Value*) {
    if (args.empty())          std::exit(0);
    if (is_boolean(args[0]))   std::exit(as_boolean(args[0]) ? 0 : 1);
    if (is_integer(args[0]))   std::exit(static_cast<int>(as_integer(args[0])));
    std::exit(1);
}
...
static Value _prim_emergency_exit(Context*, Environment*, std::vector<Value>& args, const Value*) {
    ...
    if (args.empty())          std::_Exit(0);
    if (is_boolean(args[0]))   std::_Exit(as_boolean(args[0]) ? 0 : 1);
    if (is_integer(args[0]))   std::_Exit(static_cast<int>(as_integer(args[0])));
    std::_Exit(1);
}
...
static Value _prim_get_environment_variable(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (!val) return make_boolean(false);
...
static Value _prim_get_environment_variables(Context*, Environment*, std::vector<Value>&, const Value*) {
    ...
    if (!block) return NIL_VALUE;
    std::vector<std::pair<std::string,std::string>> entries;
    for (char* p = block; *p; p += strlen(p) + 1) {
        std::string entry(p);
        auto eq = entry.find('=');
        if (eq == 0) continue;  // skip hidden Win32 env vars like "=C:=..."
