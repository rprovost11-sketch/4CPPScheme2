...
static bool _i64_mul(int64_t a, int64_t b, int64_t& out) {
    ...
    if (a == -1) { out = -b; return (b == INT64_MIN); }
    if (b == -1) { out = -a; return (a == INT64_MIN); }
...
static Value _mpz_to_value(const __mpz_struct* z) {
    ...
    if (mpz_sizeinbase(z, 2) <= 63) {
        char buf[24]; mpz_get_str(buf, 10, z);
        errno = 0;
        int64_t n = std::strtoll(buf, nullptr, 10);
        if (errno == 0) return make_integer(n);
    }
...
static Rat _as_exact_component(const Value& v) {
    if (is_integer(v)) return Rat(as_integer(v));
...
static double _numany_to_double(const NumAny& n) {
    return std::visit([](auto x) -> double { return static_cast<double>(x); }, n);
}
...
static Rat _numany_to_rat(const NumAny& n) {
    return std::visit([](auto x) -> Rat {
        ...
        else                                          return Rat::from_float(x);
    }, n);
}

static NumAny _numany_add(const NumAny& a, const NumAny& b) {
    return std::visit([](auto x, auto y) -> NumAny { return x + y; }, a, b);
}
...
static NumAny _numany_neg(const NumAny& a) {
    return std::visit([](auto x) -> NumAny { return -x; }, a);
}

static bool _numany_lt(const NumAny& a, const NumAny& b) {
    return std::visit([](auto x, auto y) -> bool { return x < y; }, a, b);
}

static bool _is_exact_zero(const NumAny& n) {
    if (auto* i = std::get_if<int64_t>(&n)) return *i == 0;
    if (auto* r = std::get_if<Rat>(&n))     return r->numerator == 0;
    return false;
}
...
static Value _wrap_cplx_result(const NumAny& re, const NumAny& im, bool exact) {
    if (exact) {
        ...
        if (im_f == 0.0) return make_real(re_f);
...
static std::complex<double> _val_to_complex(const Value& v) {
    ...
    if (is_integer(v))  return { static_cast<double>(as_integer(v)), 0.0 };
    if (is_rational(v)) return { static_cast<double>(Rat(as_rational_num(v), as_rational_den(v))), 0.0 };
    if (is_real(v))     return { as_real(v), 0.0 };
    return { 0.0, 0.0 };
}

static Value _wrap_cplx(const std::complex<double>& c) {
    if (c.imag() == 0.0) return make_real(c.real());
...
static NumAny _any_num(const Value& v, const char* name, const Value* app, int i) {
    ...
    if (is_rational(v))      return Rat(as_rational_num(v), as_rational_den(v));
    if (is_real(v))          return as_real(v);
    if (is_complex(v))       return as_complex_real(v);  // float approximation
    if (is_exact_complex(v)) return static_cast<double>(_as_exact_component(as_exact_complex_real(v)));
    throw SchemeTypeError(
...
static bool _has_any_complex(const std::vector<Value>& args) {
    for (const auto& a : args)
        if (is_complex(a) || is_exact_complex(a)) return true;
    return false;
}
...
static IntXResult _check_int_x(const Value& v, const char* name, const Value* app, int i) {
    if (is_integer(v)) return { as_integer(v), true };
    if (is_real(v)) {
        double fv = as_real(v);
...
static int64_t _trunc_div(int64_t n, int64_t d) { return n / d; }
...
static std::string _format_real(double f) {
    if (std::isnan(f)) return "+nan.0";
    if (f == std::numeric_limits<double>::infinity())  return "+inf.0";
    if (f == -std::numeric_limits<double>::infinity()) return "-inf.0";
    ...
    for (int prec = 1; prec <= 17; ++prec) {
        ...
        if (res.ec == std::errc{} && check == f) { best = std::string(buf, n); break; }
    }
    if (best.empty()) {
    ...
    if (best.find('e') != std::string::npos && af >= 1e-4 && af < 1e16) {
        ...
        if (dp != std::string::npos) {
            size_t last = d.size();
            while (last > dp + 2 && d[last - 1] == '0') --last;
            d.resize(last);
        } else { d += ".0"; }
        best = d;
    }
...
static bool _parse_real_stn(const std::string& s, double& out) {
    ...
    if (s == "-inf.0")  { out = -std::numeric_limits<double>::infinity();  return true; }
    if (s == "+nan.0" || s == "-nan.0") { out = std::numeric_limits<double>::quiet_NaN(); return true; }
    try {
        size_t pos;
        out = std::stod(s, &pos);
        return pos == s.size();
...
static bool _parse_complex_stn(const std::string& s, double& re_out, double& im_out) {
    ...
    for (int j = (int)body.size() - 1; j >= 0; --j) {
        char c = body[j];
        if (c == '+' || c == '-') {
            if (j > 0 && std::tolower((unsigned char)body[j - 1]) == 'e') { --j; continue; }
            split = j; break;
        }
    }
    ...
    double re = 0.0;
    if (!real_str.empty() && !_parse_real_stn(real_str, re)) return false;
    double im;
    if (sign_imag == "+")      im = 1.0;
    else if (sign_imag == "-") im = -1.0;
    else if (!_parse_real_stn(sign_imag, im)) return false;
    re_out = re; im_out = im;
    return true;
}
...
static Value _prim_add(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    for (auto& a : args) if (!is_integer(a)) { all_fixnum = false; break; }
    if (all_fixnum) {
        int64_t sum = 0;
        for (size_t i = 0; i < args.size(); ++i) {
            int64_t out;
            if (_i64_add(sum, as_integer(args[i]), out)) {
                ...
                __mpz_struct acc; mpz_init_set_si(&acc, 0);
                for (auto& a : args) {
                    __mpz_struct tmp; _val_to_mpz(&tmp, a);
                    mpz_add(&acc, &acc, &tmp); mpz_clear(&tmp);
                }
                Value result = _mpz_to_value(&acc); mpz_clear(&acc);
                return result;
            }
            sum = out;
        }
    ...
    for (auto& a : args) if (is_bignum(a)) { has_bignum = true; break; }
    if (has_bignum) {
        // If mix of bignum/fixnum only, use mpz.
        bool all_int = true;
        for (auto& a : args) if (!is_integer(a) && !is_bignum(a)) { all_int = false; break; }
        if (all_int) {
            __mpz_struct acc; mpz_init_set_si(&acc, 0);
            for (auto& a : args) {
                __mpz_struct tmp; _val_to_mpz(&tmp, a);
...
static Value _prim_sub(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    auto all_fixnum_or_bignum = [&]() {
        for (auto& a : args) if (!is_integer(a) && !is_bignum(a)) return false;
        return true;
    };
    if (all_fixnum_or_bignum()) {
        if (args.size() == 1) {
            if (is_integer(args[0])) {
                ...
                if (n != INT64_MIN) return make_integer(-n);
                return make_bignum_si(-static_cast<double>(n) > 0 ? 1 : -1);  // promote
            }
            __mpz_struct z; _val_to_mpz(&z, args[0]);
            mpz_neg(&z, &z);
            Value r = _mpz_to_value(&z); mpz_clear(&z); return r;
        }
        __mpz_struct acc; _val_to_mpz(&acc, args[0]);
        ...
        if (!needed_bignum) {
            ...
            bool overflow = false;
            for (size_t i = 1; i < args.size(); ++i) {
                if (is_bignum(args[i])) { overflow = true; break; }
                int64_t out;
                if (_i64_sub(result, as_integer(args[i]), out)) { overflow = true; break; }
                result = out;
            }
            if (!overflow) { mpz_clear(&acc); return make_integer(result); }
        }
        for (size_t i = 1; i < args.size(); ++i) {
            __mpz_struct tmp; _val_to_mpz(&tmp, args[i]);
            mpz_sub(&acc, &acc, &tmp); mpz_clear(&tmp);
        }
        Value r = _mpz_to_value(&acc); mpz_clear(&acc); return r;
    }
    if (args.size() == 1)
...
static Value _prim_mul(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (all_int) {
        ...
        if (!has_bignum) {
            ...
            for (auto& a : args) {
                int64_t out;
                if (_i64_mul(prod, as_integer(a), out)) { overflow = true; break; }
                prod = out;
            }
            if (!overflow) return make_integer(prod);
        }
        // mpz path.
        __mpz_struct acc; mpz_init_set_si(&acc, 1);
        for (auto& a : args) {
            __mpz_struct tmp; _val_to_mpz(&tmp, a);
            mpz_mul(&acc, &acc, &tmp); mpz_clear(&tmp);
        }
        Value result = _mpz_to_value(&acc); mpz_clear(&acc);
        return result;
    }
...
static Value _prim_div(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    if (_has_any_complex(args)) {
        ...
        if (args.size() == 1) {
            ...
            if (!acc.valid) throw SchemeTypeError("/: division by zero", _src(app));
            return _wrap_cplx_result(acc.re, acc.im, acc.exact);
        }
        for (size_t i = 1; i < args.size(); ++i) {
    ...
    if (args.size() == 1) {
        ...
        if (_is_exact_zero(n)) throw SchemeTypeError("/: division by zero", _src(app));
        try { return _wrap_numany(_exact_div_numany(NumAny{int64_t(1)}, n)); }
        catch (std::domain_error&) { throw SchemeTypeError("/: division by zero", _src(app)); }
    }
    NumAny result = _any_num(args[0], "/", app, 1);
    for (size_t i = 1; i < args.size(); ++i) {
        NumAny divisor = _any_num(args[i], "/", app, (int)(i + 1));
        if (_is_exact_zero(divisor)) throw SchemeTypeError("/: division by zero", _src(app));
        try { result = _exact_div_numany(result, divisor); }
        catch (std::domain_error&) { throw SchemeTypeError("/: division by zero", _src(app)); }
    }
    return _wrap_numany(result);
}
...
static Value _prim_quotient(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("quotient: division by zero", _src(app));
...
static Value _prim_remainder(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("remainder: division by zero", _src(app));
...
static Value _prim_modulo(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("modulo: division by zero", _src(app));
    int64_t r = n.re % d.re;
    // Adjust: result must have sign of d (Python semantics).
    if (r != 0 && ((r < 0) != (d.re < 0))) r += d.re;
    return (!n.exact || !d.exact) ? make_real((double)r) : make_integer(r);
}
...
static Value _prim_min(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    for (size_t i = 1; i < args.size(); ++i) {
        ...
        if (is_real(args[i])) any_real = true;
        if (std::holds_alternative<double>(v) && std::isnan(std::get<double>(v))) {
            ...
            if (_numany_lt(v, result)) result = v;
        }
    }
...
static Value _prim_max(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    for (size_t i = 1; i < args.size(); ++i) {
        ...
        if (std::holds_alternative<double>(v) && std::isnan(std::get<double>(v))) {
            ...
            if (_numany_lt(result, v)) result = v;
        }
    }
    if (any_real && !std::holds_alternative<double>(result))
        result = _numany_to_double(result);
...
static Value _prim_gcd(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    if (args.empty()) return make_integer(0);
...
static Value _prim_lcm(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    if (args.empty()) return make_integer(1);
...
static Value _prim_expt(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if ((is_integer(base_v) || is_bignum(base_v)) && is_integer(exp_v)) {
        ...
        if (e >= 0) {
            ...
            if (pos.numerator == 0) throw SchemeTypeError("expt: division by zero", _src(app));
...
static Value _prim_sqrt(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (is_integer(v)) {
        ...
        if (n >= 0) {
            ...
            while (r > 0 && r * r > n) --r;
            while ((r + 1) * (r + 1) <= n) ++r;
            if (r * r == n) return make_integer(r);
        }
    }
    if (is_rational(v)) {
        ...
        if (num >= 0) {
            ...
            while (rn > 0 && rn * rn > num) --rn;
            while ((rn + 1) * (rn + 1) <= num) ++rn;
            while (rd > 0 && rd * rd > den) --rd;
            while ((rd + 1) * (rd + 1) <= den) ++rd;
    ...
    // Float path.
    double fv;
    if (is_integer(v))       fv = (double)as_integer(v);
    else if (is_rational(v)) fv = as_rational_num(v) / (double)as_rational_den(v);
    else if (is_real(v))     fv = as_real(v);
    else throw SchemeTypeError("sqrt: argument must be a number", _src(app));
    if (fv == 0.0) return make_real(0.0);
    if (fv < 0.0)  return _wrap_cplx(std::sqrt(std::complex<double>(fv, 0.0)));
...
static Value _prim_exact(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_integer(v) || is_bignum(v) || is_rational(v) || is_exact_complex(v)) return v;
    if (is_real(v)) return _float_to_exact(as_real(v), "exact", app);
    if (is_complex(v)) {
        Value re = _float_to_exact(as_complex_real(v), "exact", app);
        double im_f = as_complex_imag(v);
        if (im_f == 0.0) return re;
...
static Value _prim_inexact(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_real(v) || is_complex(v)) return v;
    if (is_integer(v))  return make_real((double)as_integer(v));
    if (is_bignum(v))   return make_real(mpz_get_d(as_bignum(v)));
    if (is_rational(v)) return make_real(as_rational_num(v) / (double)as_rational_den(v));
    if (is_exact_complex(v)) {
        double re = static_cast<double>(_as_exact_component(as_exact_complex_real(v)));
        double im = static_cast<double>(_as_exact_component(as_exact_complex_imag(v)));
...
static Value _prim_numerator(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_integer(v) || is_bignum(v))  return v;
    if (is_rational(v)) return make_integer(as_rational_num(v));
    if (is_real(v)) {
        double f = as_real(v);
        if (!std::isfinite(f)) return make_real(f);
        if (f == std::trunc(f)) return make_real(f);
...
static Value _prim_denominator(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_integer(v) || is_bignum(v))  return make_integer(1);
    if (is_rational(v)) return make_integer(as_rational_den(v));
    if (is_real(v)) {
        double f = as_real(v);
        if (!std::isfinite(f)) return make_real(1.0);
        if (f == std::trunc(f)) return make_real(1.0);
...
static Value _prim_number_to_string(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (is_integer(v)) {
        int64_t n = as_integer(v);
        if (radix == 10) return make_string(std::to_string(n));
    ...
    if (is_complex(v)) {
        ...
        if (std::isnan(im) || im >= 0) return make_string(re_s + "+" + im_s + "i");
    ...
    if (is_exact_complex(v)) {
        ...
        if (im_r >= int64_t(0)) return make_string(re_s + "+" + im_s + "i");
        return make_string(re_s + im_s + "i");
    }
...
static Value _prim_string_to_number(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    while (s.size() >= 2 && s[0] == '#') {
        ...
        if (ch == 'b') {
            if (explicit_radix) return make_boolean(false);
            radix = 2; explicit_radix = true; s = s.substr(2);
        } else if (ch == 'o') {
            if (explicit_radix) return make_boolean(false);
            radix = 8; explicit_radix = true; s = s.substr(2);
        } else if (ch == 'd') {
            if (explicit_radix) return make_boolean(false);
            radix = 10; explicit_radix = true; s = s.substr(2);
        } else if (ch == 'x') {
            if (explicit_radix) return make_boolean(false);
            radix = 16; explicit_radix = true; s = s.substr(2);
        } else if (ch == 'e') {
            if (exact != -1) return make_boolean(false);
            exact = 1; s = s.substr(2);
        } else if (ch == 'i') {
            if (exact != -1) return make_boolean(false);
            exact = 0; s = s.substr(2);
        } else {
            return make_boolean(false);
        }
    }
    if (s.empty()) return make_boolean(false);
    ...
    if (radix == 10 && s.size() >= 2 && s.back() == 'i') {
        ...
        if (_parse_complex_stn(s, re_f, im_f)) {
            if (exact == 1) {
                ...
                if (is_integer(im_v) && as_integer(im_v) == 0) return re_v;
    ...
    try {
        ...
        if (pos == s.size()) {
            if (exact == 0) return make_real((double)n);
    ...
    if (s.find('/') != std::string::npos) {
        ...
        try {
            ...
            if (den != 0) {
                Rat frac(num, den);
                if (exact == 0) return make_real(static_cast<double>(frac));
                if (frac.denominator == 1) return make_integer(frac.numerator);
                return make_rational(frac.numerator, frac.denominator);
            }
        } catch (...) {}
    }
    ...
    if (radix == 10) {
        ...
        if (_parse_real_stn(s, f)) {
            if (exact == 1) {
                if (!std::isfinite(f)) return make_boolean(false);
...
static Value _prim_floor_quotient(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("floor-quotient: divide by zero", _src(app));
    int64_t r = n.re / d.re;
    // Adjust toward -inf for floor division.
    if ((n.re ^ d.re) < 0 && r * d.re != n.re) --r;
    return (!n.exact || !d.exact) ? make_real((double)r) : make_integer(r);
}
...
static Value _prim_floor_remainder(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("floor-remainder: divide by zero", _src(app));
    int64_t r = n.re % d.re;
    if (r != 0 && ((r < 0) != (d.re < 0))) r += d.re;
    return (!n.exact || !d.exact) ? make_real((double)r) : make_integer(r);
}
...
static Value _prim_floor_div(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("floor/: divide by zero", _src(app));
    int64_t q = n.re / d.re;
    if ((n.re ^ d.re) < 0 && q * d.re != n.re) --q;
...
static Value _prim_truncate_quotient(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("truncate-quotient: divide by zero", _src(app));
    int64_t r = _trunc_div(n.re, d.re);
    return (!n.exact || !d.exact) ? make_real((double)r) : make_integer(r);
}
...
static Value _prim_truncate_remainder(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("truncate-remainder: divide by zero", _src(app));
...
static Value _prim_truncate_div(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (d.re == 0) throw SchemeTypeError("truncate/: divide by zero", _src(app));
...
static Value _prim_exact_integer_sqrt(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (n < 0) throw SchemeTypeError("exact-integer-sqrt: argument must be non-negative", _src(app));
    int64_t r = (int64_t)std::sqrt((double)n);
    while (r > 0 && r * r > n) --r;
    while ((r + 1) * (r + 1) <= n) ++r;
...
static Value _prim_exp(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (_is_complex_type(v)) return _wrap_cplx(std::exp(_val_to_complex(v)));
...
static Value _prim_sin(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (_is_complex_type(v)) return _wrap_cplx(std::sin(_val_to_complex(v)));
...
static Value _prim_cos(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (_is_complex_type(v)) return _wrap_cplx(std::cos(_val_to_complex(v)));
...
static Value _prim_tan(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (_is_complex_type(v)) return _wrap_cplx(std::tan(_val_to_complex(v)));
...
static Value _prim_asin(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (_is_complex_type(v)) return _wrap_cplx(std::asin(_val_to_complex(v)));
    double f = _numany_to_double(_any_num(v, "asin", app, 1));
    if (f < -1.0 || f > 1.0) return _wrap_cplx(std::asin(std::complex<double>(f, 0.0)));
    return make_real(std::asin(f));
}

static Value _prim_acos(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (_is_complex_type(v)) return _wrap_cplx(std::acos(_val_to_complex(v)));
    double f = _numany_to_double(_any_num(v, "acos", app, 1));
    if (f < -1.0 || f > 1.0) return _wrap_cplx(std::acos(std::complex<double>(f, 0.0)));
...
static Value _prim_make_rectangular(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (re_exact && im_exact) {
        ...
        if (im_r == int64_t(0)) return re;
        return make_exact_complex(re, im);
    }
    double re_f, im_f;
    if (is_integer(re))       re_f = (double)as_integer(re);
    else if (is_rational(re)) re_f = as_rational_num(re) / (double)as_rational_den(re);
    else if (is_real(re))     re_f = as_real(re);
    else throw SchemeTypeError("make-rectangular: argument 1 is not a real number", _src(app));
    if (is_integer(im))       im_f = (double)as_integer(im);
    else if (is_rational(im)) im_f = as_rational_num(im) / (double)as_rational_den(im);
    else if (is_real(im))     im_f = as_real(im);
    else throw SchemeTypeError("make-rectangular: argument 2 is not a real number", _src(app));
    return make_complex(re_f, im_f);
}
...
static Value _prim_real_part(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_exact_complex(v)) return as_exact_complex_real(v);
    if (is_complex(v))       return make_real(as_complex_real(v));
    if (is_integer(v) || is_bignum(v) || is_rational(v) || is_real(v)) return v;
...
static Value _prim_imag_part(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    const Value& v = args[0];
    if (is_exact_complex(v)) return as_exact_complex_imag(v);
    if (is_complex(v))       return make_real(as_complex_imag(v));
    if (is_integer(v) || is_bignum(v) || is_rational(v)) return make_integer(0);
    if (is_real(v))          return make_real(0.0);
...
static Value _prim_rationalize(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (auto* fp = std::get_if<double>(&x)) {
        if (!std::isfinite(*fp)) return make_real(*fp);
    }
    if (auto* fp = std::get_if<double>(&delta)) {
        if (std::isinf(*fp)) return inexact ? make_real(0.0) : make_integer(0);
        if (std::isnan(*fp)) return make_real(std::numeric_limits<double>::quiet_NaN());
    }
    ...
    if (inexact) {
        double xd = std::visit([](auto v) -> double { return static_cast<double>(v); }, x);
        double dd = std::visit([](auto v) -> double { return static_cast<double>(v); }, delta);
        if (dd < 0.0) dd = -dd;
        double lo_d = xd - dd, hi_d = xd + dd;
        Rat rlo = Rat::from_float(lo_d), rhi = Rat::from_float(hi_d);
        Rat r;
        if (rlo <= Rat(0) && rhi >= Rat(0)) r = Rat(0);
        else if (rlo > Rat(0)) r = _simplest_rational(rlo, rhi);
        else r = -_simplest_rational(-rhi, -rlo);
...
static Value _prim_random(Context*, Environment*, std::vector<Value>& args, const Value* app) {
    ...
    if (is_integer(n)) {
        ...
        if (v <= 0) throw SchemeTypeError("random: argument must be positive", _src(app));
        std::uniform_int_distribution<int64_t> dist(0, v - 1);
        return make_integer(dist(rng));
    }
    if (is_real(n)) {
        double v = as_real(n);
        if (v <= 0.0) throw SchemeTypeError("random: argument must be positive", _src(app));
