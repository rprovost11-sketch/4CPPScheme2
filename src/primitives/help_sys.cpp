...
struct Layout { int nrows, ncols; std::vector<int> colwidths; };
...
static std::vector<std::string> _columnize(const std::vector<std::string>& items, int width = 78) {
    ...
    for (int nrows = 1; nrows <= static_cast<int>(items.size()); ++nrows) {
        ...
        if (layout) { best = layout; break; }
    }
    if (!best) {
        int widest = 0;
        for (auto& s : items) if (static_cast<int>(s.size()) > widest) widest = static_cast<int>(s.size());
...
static std::unordered_map<std::string, std::vector<std::string>> _discover_topics() {
    ...
    if (!std::filesystem::is_directory(hdir)) return topics;
    _scan_dir(hdir, "", topics);
    for (auto& [sec, stems] : topics) std::sort(stems.begin(), stems.end());
    return topics;
}
    ...
    for (auto& entry : std::filesystem::directory_iterator(root, ec)) {
        if (entry.is_directory()) {
            ...
            if (name.size() > 4 && name.substr(name.size() - 4) == ".txt") {
                ...
                if (lo_stem == lo_name) return entry.path().string();
            }
        }
    }
    return {};
...
static std::string _find_topic(const std::string& name) {
    std::filesystem::path hdir(_help_dir());
    if (!std::filesystem::is_directory(hdir)) return {};
    std::string lo = name;
...
static std::string _type_label(const std::string& kind) {
    if (kind == "special")   return "Special Form";
...
static std::string _args_label(const std::string& kind) {
    if (kind == "special" || kind == "macro") return "args: unevaluated";
...
static std::vector<std::string> _collect_closures(Environment* global_env) {
    ...
    for (auto& [sid, val] : global_env->_bindings) {
        if (is_closure(val)) closures.push_back(symbol_name(sid));
    }
...
static std::vector<std::string> _category_print_order(
    ...
    for (auto& cat : order_list) {
        if (by_cat.count(cat)) { order.push_back(cat); seen[cat] = true; }
    }
    std::vector<std::string> remaining;
    for (auto& [cat, _] : by_cat) {
        if (!seen.count(cat)) remaining.push_back(cat);
    }
    std::sort(remaining.begin(), remaining.end());
    for (auto& cat : remaining) order.push_back(cat);
    return order;
}

static void _write_doc_block(Context* ctx, const std::string& doc) {
    if (doc.empty()) { ctx->write("   (no documentation)\n"); return; }
    std::istringstream ss(doc);
    std::string line;
    while (std::getline(ss, line)) {
        if (!line.empty()) ctx->write("   " + line + "\n");
        else ctx->write("\n");
    }
}
...
static void _write_topics_section(Context* ctx,
    ...
    for (auto& section : section_keys) {
        ...
        if (!section.empty()) {
            ...
            for (auto& line : _columnize(items)) ctx->write(line + "\n");
        }
    }
    ctx->write("\n");
}
...
static void _list_all(Context* ctx, Environment* env) {
    ...
    for (auto& cat : _category_print_order(by_cat)) {
        ...
        if (it != cat_titles.end()) title = it->second;
        else { title = cat; if (!title.empty()) title[0] = static_cast<char>(std::toupper(static_cast<unsigned char>(title[0]))); }
        ctx->write(title + "\n");
        ctx->write(std::string(title.size(), '-') + "\n");
        for (auto& line : _columnize(by_cat[cat])) ctx->write(line + "\n");
        ctx->write("\n");
    }
    ...
    if (!closures.empty()) {
        ...
        ctx->write("----------\n");
        for (auto& line : _columnize(closures)) ctx->write(line + "\n");
        ctx->write("\n");
    }

    auto topics = _discover_topics();
    if (!topics.empty()) _write_topics_section(ctx, topics);
...
static std::string _closure_usage(const std::string& name, const Value& closure) {
    ...
    if (rest_id != UINT32_MAX) {
        if (parts.empty()) return "(" + name + " . " + rest_str + ")";
...
static std::vector<std::string> _collect_matching_names(Environment* global_env, const std::string& lo) {
    ...
    for (auto& [sid, val_] : global_env->_bindings) {
        ...
        if (lo_name.find(lo) != std::string::npos) result.push_back(name);
    }
...
static std::vector<std::string> _collect_matching_topics(
    ...
    for (auto& [k, _] : topics) keys.push_back(k);
    std::sort(keys.begin(), keys.end());
    for (auto& section : keys) {
        for (auto& stem : topics.at(section)) {
            std::string lo_stem = stem;
            std::transform(lo_stem.begin(), lo_stem.end(), lo_stem.begin(), ::tolower);
            if (lo_stem.find(lo) != std::string::npos) result.push_back("\"" + stem + "\"");
        }
    }
    std::sort(result.begin(), result.end());
    return result;
...
static void _apropos(Context* ctx, Environment* env, const std::string& pattern) {
    ...
    if (!names.empty()) {
        ctx->write("Names\n");
        ctx->write("-----\n");
        for (auto& line : _columnize(names)) ctx->write(line + "\n");
        ctx->write("\n");
    }
    if (!topic_matches.empty()) {
        ctx->write("Topics\n");
        ctx->write("------\n");
        for (auto& line : _columnize(topic_matches)) ctx->write(line + "\n");
        ctx->write("\n");
    }
}
...
static Value _prim_help(Context* ctx, Environment* env, std::vector<Value>& args, const Value* app) {
    if (args.empty()) { _list_all(ctx, env); return VOID_VALUE; }
    const Value& target = args[0];

    if (is_string(target)) {
        const std::string& name = as_string(target);
        if (!_show_topic(ctx, name)) _apropos(ctx, env, name);
        return VOID_VALUE;
    }
    ...
    if (is_closure(target)) {
        ...
        for (auto& [sid, val] : global_env->_bindings) {
            if (_val_equal(val, target)) { matched_name = symbol_name(sid); break; }
        }
    ...
    if (is_symbol(target)) {
        ...
        if (!val_opt) { ctx->write("No binding for '" + name + "'.\n"); return VOID_VALUE; }
