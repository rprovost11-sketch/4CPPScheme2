...
static bool starts_like_number(const std::string& s)
   ...
   if ((s[0] == '+' || s[0] == '-') && s.size() > 1)
      ...
      if (s[1] >= '0' && s[1] <= '9')
         return true;
      if (s[1] == '.' && s.size() > 2 && s[2] >= '0' && s[2] <= '9')
         return true;
      }
   if (s[0] == '.' && s.size() > 1 && s[1] >= '0' && s[1] <= '9')
      return true;
   return false;
   }
...
static double comp_to_double(const ComplexComp& v)
   {
   if (auto* ip = std::get_if<int64_t>(&v))
      return (double)*ip;
   if (auto* rp = std::get_if<Rat>(&v))
      return static_cast<double>(*rp);
...
static std::optional<ComplexComp> parse_num_for_complex(const std::string& s)
   ...
   if (s.empty())
      return std::nullopt;
   if (s == "+inf.0")
      return ComplexComp(double(INFINITY));
   if (s == "-inf.0")
      return ComplexComp(double(-INFINITY));
   if (s == "+nan.0" || s == "-nan.0")
      return ComplexComp(double(NAN));
      // Try integer
      {
      ...
      if (st < s.size())
         ...
         if (ok)
            ...
            try
               {
               return ComplexComp(std::stoll(s));
               }
            catch (...)
               {
               }
...
static Rat decimal_str_to_rat(const std::string& s)
   ...
   int64_t den = 1;
   for (int64_t k = 0; k < frac_digits; ++k)
      den *= 10;
   if (exp > 0)
      {
...
static bool valid_uint_radix(const std::string& s, int radix)
   ...
   for (char c : s)
      ...
      int d = (lc >= '0' && lc <= '9') ? (lc - '0') : (lc >= 'a' && lc <= 'z') ? (lc - 'a' + 10)
                                                                               : radix;
      if (d >= radix)
         return false;
      }
...
static Token try_parse_prefixed_number(const std::string& text, const SourceInfo& src)
   ...
   while (i < text.size() && text[i] == '#')
      ...
      if (ch == 'b')
         {
         if (radix != -1)
            throw SchemeSyntaxError("duplicate radix prefix: '" + text + "'", new SourceInfo(src));
         radix = 2;
         i += 2;
         }
      else if (ch == 'o')
         ...
         {
         if (exact != -1)
            throw SchemeSyntaxError("duplicate exactness prefix: '" + text + "'", new SourceInfo(src));
         exact = 1;
         i += 2;
         }
      else if (ch == 'i')
            ...
            try
               ...
               if (den != 0)
                  ...
                  if (exact == 0)
                     {
                     Token t(TokenKind::REAL, src);
                     t.dbl_val = static_cast<double>(frac);
                     return t;
                     }
   ...
   if (radix == 10)
      ...
      if (rest == "+inf.0")
         {
         if (exact == 1)
            throw SchemeSyntaxError("#e cannot be applied to +inf.0", new SourceInfo(src));
         Token t(TokenKind::REAL, src);
         t.dbl_val = INFINITY;
         return t;
         }
...
std::vector<Token> scheme_tokenize(const std::string& source, const std::string& filename)
   ...
   auto cur_src = [&]()
   { return make_src_val(line, col, source_lines, filename); };
   ...
   while (pos < n)
      ...
      if (source[pos] == '|')
         ...
         if (fold_case)
            for (char& c : name)
               c = (char)std::tolower((unsigned char)c);
         Token tok(TokenKind::IDENT, sv);
         tok.str_val = std::move(name);
         tokens.push_back(std::move(tok));
         continue;
         }
      ...
      if (c == '(')
         {
         tokens.emplace_back(TokenKind::LPAREN, sv);
         ++pos;
         ++col;
         continue;
         }
      if (c == ')')
         {
         tokens.emplace_back(TokenKind::RPAREN, sv);
         ++pos;
         ++col;
         continue;
         }
      if (c == '[')
         {
         tokens.emplace_back(TokenKind::LBRACKET, sv);
         ++pos;
         ++col;
         continue;
         }
      if (c == ']')
         {
         tokens.emplace_back(TokenKind::RBRACKET, sv);
         ++pos;
         ++col;
         continue;
         }
      ...
      if (c == '\'')
         {
         tokens.emplace_back(TokenKind::QUOTE, sv);
         ++pos;
         ++col;
         continue;
         }
      if (c == '`')
         {
         tokens.emplace_back(TokenKind::QUASIQUOTE, sv);
         ++pos;
         ++col;
         continue;
         }
      if (c == ',')
         ...
         if (pos + 1 < n && source[pos + 1] == '@')
            {
            tokens.emplace_back(TokenKind::UNQUOTE_SPLICING, sv);
            pos += 2;
            col += 2;
            }
         else
            {
            tokens.emplace_back(TokenKind::UNQUOTE, sv);
            ++pos;
            ++col;
            }
         continue;
      ...
      if (c == '"')
         ...
         Token tok(TokenKind::STRING, sv);
         tok.str_val = std::move(content);
         tokens.push_back(std::move(tok));
         continue;
         }
      ...
      if (c == '#')
         ...
         if (pos + 3 < n && source[pos + 1] == 'u' && source[pos + 2] == '8' && source[pos + 3] == '(')
            {
            tokens.emplace_back(TokenKind::BYTEVECTOR_LPAREN, sv);
            pos += 4;
            col += 4;
            continue;
            }
         ...
         if (pos + 1 < n && source[pos + 1] == '(')
            {
            tokens.emplace_back(TokenKind::VECTOR_LPAREN, sv);
            pos += 2;
            col += 2;
            continue;
            }
         ...
         if (pos + 1 < n && (source[pos + 1] == 't' || source[pos + 1] == 'T'))
            {
            size_t rp = pos + 2;
            if (rp + 3 <= n && source.substr(rp, 3) == "rue")
               rp += 3;
            if (rp >= n || is_delim(source[rp]))
               {
               Token tok(TokenKind::BOOL, sv);
               tok.bool_val = true;
               tokens.push_back(std::move(tok));
               col += (int)(rp - pos);
               pos = rp;
         ...
         if (pos + 1 < n && (source[pos + 1] == 'f' || source[pos + 1] == 'F'))
            {
            size_t rp = pos + 2;
            if (rp + 4 <= n && source.substr(rp, 4) == "alse")
               rp += 4;
            if (rp >= n || is_delim(source[rp]))
               {
               Token tok(TokenKind::BOOL, sv);
      ...
      if (c == '.')
         ...
         if (pos + 1 >= n || is_delim(source[pos + 1]))
            {
            tokens.emplace_back(TokenKind::DOT, sv);
            ++pos;
            ++col;
            continue;
            }
         ...
         {
         size_t ws = pos;
         while (pos < n && !is_ident_delim(source[pos]))
            ++pos;
         if (pos == ws)
...
class SchemeParser
   ...
   const Token& _peek() const
      {
      return tokens_[pos_];
      } // public: used by scheme_parse_one

   void _advance()
      {
      if (tokens_[pos_].kind != TokenKind::END_OF_FILE)
         ++pos_;
      }
 ...
 public:
   ...
   std::vector<Value> parse_program()
      ...
      while (true)
         {
         _skip_datum_comments();
         if (_peek().kind == TokenKind::END_OF_FILE)
            break;
