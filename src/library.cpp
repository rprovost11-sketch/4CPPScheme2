   ...
   if (is_symbol(head))
      ...
      if (name == "rename")
         ...
         while (is_cons(pairs_cur))
            ...
            if (!is_cons(pair) || !is_symbol(car(pair)) || !is_cons(cdr(pair)) || !is_symbol(car(cdr(pair))))
               throw std::runtime_error(
      ...
      if (name == "prefix")
         {
         if (!is_cons(tail) || !is_cons(cdr(tail)) || !is_symbol(car(cdr(tail))))
            throw std::runtime_error(
...
static const std::vector<std::string> _SCHEME_BASE_NAMES = {
    ...
    // Characters in (scheme base) (the ci / case / predicate variants live in
    // (scheme char)).
    "char->integer",
    "integer->char",
    "char?",
    ...
    "utf8->string",
    "string->utf8",
    // Other (scheme base) procedures missing from the export list.
    "exact-integer-sqrt",
    "file-error?",
    "read-error?",
    "features",
    // Input and output (R7RS 6.13).  These exist as global primitives but were not
    // exported by any library, so define-library code (e.g. an SRFI written in Scheme)
    // could not reach them; (scheme base) is their R7RS home.  Names not implemented
    // are silently skipped by _register_filtered.
    "call-with-port",
    "input-port?",
    "output-port?",
    "textual-port?",
