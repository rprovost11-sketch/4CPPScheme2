...
struct EnterResult {
    enum Kind { IsValue, IsCont, IsEnter } kind = IsValue;
    Value        v;
...
static Value make_closure_from_lambda(const Value& lam, Environment* env) {
    ...
    if (is_symbol(params_sexpr)) {
        ...
        while (is_cons(cur)) { params.push_back(as_symbol_id(car(cur))); cur = cdr(cur); }
        if (is_symbol(cur)) rest_id = as_symbol_id(cur);
    }
...
static Value make_case_closure_from_form(const Value& cl_cons, Environment* env) {
    ...
    while (is_cons(cur)) {
        ...
        if (is_symbol(ps)) {
            ...
            Value p = ps;
            while (is_cons(p)) { cl.params.push_back(as_symbol_id(car(p))); p = cdr(p); }
            if (is_symbol(p)) cl.rest_name_id = as_symbol_id(p);
        }
        cl.body = cdr(clause);
...
static inline bool is_call_cc_prim(const Value& V) {
    if (!is_primitive(V)) return false;
...
static bool is_aux_keyword(const Value& sym, const std::string& name, Environment* env) {
    if (!is_symbol(sym) || as_symbol(sym) != name) return false;
    if (!env) return true;
...
static CondClause classify_cond_clause(const Value& clause, Environment* env) {
    ...
    if (is_aux_keyword(head, "else", env)) {
        CondClause cc; cc.kind = CondClause::Else; cc.body_cons = cdr(clause); return cc;
    }
    if (is_nil(cdr(clause))) {
        CondClause cc; cc.kind = CondClause::TestOnly; cc.test = head; return cc;
    }
    Value tail = cdr(clause);
    if (is_cons(tail) && is_aux_keyword(car(tail), "=>", env)
            && is_cons(cdr(tail)) && is_nil(cdr(cdr(tail)))) {
        CondClause cc; cc.kind = CondClause::Arrow;
        cc.test = head; cc.proc_expr = car(cdr(tail)); return cc;
    }
    CondClause cc; cc.kind = CondClause::Body;
    cc.test = head; cc.body_cons = cdr(clause); return cc;
}
...
static void shadow_push(Context* ctx, KStack& K, const Value* app_node) {
    ...
    if (!K.empty() && K.back().tag == FRAME_SHADOW_POP) {
        if (!ss.empty()) {
            ...
            if (top.label == label && top.src == src) { ++top.count; return; }
            ss.back() = { label, src, 1 };
        }
        return;
    }
    if ((int)ss.size() >= SHADOW_DEPTH_LIMIT) return;
    if (!ss.empty()) {
        ShadowEntry& top = ss.back();
        if (top.label == label && top.src == src) {
            ++top.count;
            Frame f; f.tag = FRAME_SHADOW_POP;
...
static Value continuation_value(const std::vector<Value>& args) {
    if (args.empty()) return VOID_VALUE;
...
static const void* val_id(const Value& v) {
    ...
    if (h) return h;
    if (is_primitive(v)) return static_cast<const void*>(&as_primitive_fn(v));
    return nullptr;
}
                               ...
                               Context* ctx, Environment* saved_env, const Value* app_node)
{
    ...
    if (pv.has_value()) {
        EnterResult r; r.kind = EnterResult::IsValue; r.v = *pv; return r;
    }
    if (is_primitive(fn_value)) {
        Value v = as_primitive_fn(fn_value)(ctx, saved_env, args, app_node);
        EnterResult r; r.kind = EnterResult::IsValue; r.v = v; return r;
    }
    ...
    const std::vector<Value>& collected, const Value* app_node)
{
    ...
    for (size_t i = 1; i + 1 < collected.size(); ++i) flat.push_back(collected[i]);
    Value last = collected.back();
    Value cur  = last;
    while (is_cons(cur)) { flat.push_back(car(cur)); cur = cdr(cur); }
    if (!is_nil(cur)) {
        SourceInfo* src = app_node ? src_of(*app_node) : nullptr;
...
static bool is_mv_ok(int ftag) {
    switch (ftag) {
    case FRAME_CWV_CONSUMER: case FRAME_SEQ: case FRAME_DYNAMIC_WIND_AFTER:
    case FRAME_POP_HANDLER: case FRAME_REINSTALL_HANDLER: case FRAME_NONCONTIN_RETURN:
    case FRAME_SHADOW_POP: case FRAME_TRACE_EXIT: case FRAME_GUARD:
        return true;
...
static std::vector<std::string> _library_load_path() {
    ...
    if (path_var == nullptr) return parts;
    ...
    Context* ctx)
{
    ...
    if (dsym == "export") {
        ...
        while (is_cons(specs)) {
            ...
            } else if (is_cons(spec) && is_symbol(car(spec))
                       && as_symbol(car(spec)) == "rename") {
                Value r = cdr(spec);
                if (!is_cons(r) || !is_symbol(car(r))
                        || !is_cons(cdr(r)) || !is_symbol(car(cdr(r))))
                    throw SchemeSyntaxError(
...
static Value cek_loop(const Value& expr, Environment* env, Context* ctx)
{
    ...
    struct HookGuard {
        ~HookGuard() { gc_pop_forward_hook(); gc_pop_trace_hook(); }
    } _hook_guard;
    ...
    auto dispatch_exc = [&](Value raised_value, bool is_scheme_raised, bool continuable) -> bool {
        ...
        if (result.kind == EnterResult::IsValue) {
            ...
            K = std::move(result.new_k); V = result.v; skip_eval = true;
        } else {
            C = result.v; E = result.new_env;
            if (is_cons(result.seq)) {
                Frame sf; sf.tag = FRAME_SEQ; sf.v1 = result.seq; sf.env = result.new_env;
    ...
    for (;;) {  // A: exception restart loop
        try {
            for (;;) {  // B: main state machine
                ...
                // ── EVAL phase (loop C) ────────────────────────────────────────
                for (;;) {
                    if (skip_eval) { skip_eval = false; break; }
                    ...
                    if (is_cons(C)) {
                        ...
                        if (is_symbol(head)) {
                            ...
                            if (sid == kw.case_lambda) {
                                V = make_case_closure_from_form(C, E); break;
                            }
                            ...
                            if (sid == kw.if_) {
                                Frame f; f.tag = FRAME_IF;
                                f.v1 = car(cdr(cdr(C)));       // then
                                f.v2 = car(cdr(cdr(cdr(C)))); // else
                                f.env = E;
                                K.push_back(std::move(f));
                                C = car(cdr(C)); continue;
                            }
                            if (sid == kw.define) {
                                Frame f; f.tag = FRAME_DEFINE;
                                f.v1 = car(cdr(C)); f.env = E;
                                K.push_back(std::move(f));
                                C = car(cdr(cdr(C))); continue;
                            }
                            if (sid == kw.set_) {
                                Value name_sexpr = car(cdr(C));
                                Frame f; f.tag = FRAME_SET;
                                f.v1 = name_sexpr; f.env = E;
                            ...
                            if (sid == kw.begin) {
                                ...
                                if (is_cons(cdr(body))) {
                                    ...
                                    f.v1 = cdr(body); f.env = E;
                                    K.push_back(std::move(f));
                                }
                                continue;
                            }
                            if (sid == kw.when) {
                                Frame f; f.tag = FRAME_WHEN;
                                f.v1 = cdr(cdr(C)); f.env = E;
                                K.push_back(std::move(f));
                                C = car(cdr(C)); continue;
                            }
                            if (sid == kw.unless) {
                                Frame f; f.tag = FRAME_UNLESS;
                                f.v1 = cdr(cdr(C)); f.env = E;
                                K.push_back(std::move(f));
                                C = car(cdr(C)); continue;
                            }
                            if (sid == kw.and_) {
                                Value body = cdr(C);
                                if (is_nil(body)) { V = make_boolean(true); break; }
                                if (is_cons(cdr(body))) {
                                    Frame f; f.tag = FRAME_AND;
                                    f.v1 = cdr(body); f.env = E;
                                    K.push_back(std::move(f));
                                }
                                C = car(body); continue;
                            ...
                            if (sid == kw.or_) {
                                Value body = cdr(C);
                                if (is_nil(body)) { V = make_boolean(false); break; }
                                if (is_cons(cdr(body))) {
                                    Frame f; f.tag = FRAME_OR;
                                    f.v1 = cdr(body); f.env = E;
                                    K.push_back(std::move(f));
                                }
                                C = car(body); continue;
                            ...
                            if (sid == kw.cond) {
                                ...
                                Frame f; f.tag = FRAME_COND;
                                f.v1 = first; f.v2 = rest; f.env = E;
                                K.push_back(std::move(f));
                                C = kind.test; continue;
                            }
                            if (sid == kw.case_) {
                                Value clauses = cdr(cdr(C));
                                Frame f; f.tag = FRAME_CASE;
                                f.v1 = car(clauses); f.v2 = cdr(clauses); f.env = E;
                                K.push_back(std::move(f));
                                C = car(cdr(C)); continue;
                            }
                            if (sid == kw.let) {
                            ...
                            if (sid == kw.letrec || sid == kw.letrec_star) {
                                ...
                                for (auto& p : pairs) new_env->bind_id(p.first, VOID_VALUE);
                                std::vector<std::pair<uint32_t,Value>> remaining(pairs.begin()+1, pairs.end());
                                Frame f; f.tag = FRAME_LETREC;
                                f.uid   = pairs[0].first;
                                f.pairs = std::move(remaining);
                                f.v1    = body_cons;
                                f.env   = new_env;
                            ...
                            if (sid == kw.trace) {
                                ...
                                if (is_nil(args_cons)) {
                                    V = sorted_sym_list(trc->get_fns()); break;
                                }
                        ...
                        // Application (keyword or non-symbol head falls through)
                        {
                            ...
                            Frame f; f.tag = FRAME_ARG;
                            f.list1 = std::move(args); f.env = E; f.v1 = C;
                            K.push_back(std::move(f));
                            C = head; continue;
                        }
                    }
                ...
                for (;;) {
                    ...
                    Frame frame = std::move(K.back()); K.pop_back();
                    int ftag = frame.tag;
                    ...
                    if (ftag == FRAME_SET) {
                        ...
                        try {
                            ...
                            if (!e.src) { SourceInfo* s = frame.src_ptr; if (s) e.src = new SourceInfo(*s); }
                            throw;
                        }
                        V = VOID_VALUE; continue;
                    }
                    ...
                    if (ftag == FRAME_CWV_CONSUMER) {
                        ...
                        if (result.kind == EnterResult::IsCont) { K = std::move(result.new_k); V = result.v; continue; }
                        C = result.v; E = result.new_env;
                        if (is_cons(result.seq)) {
                            Frame sf; sf.tag = FRAME_SEQ; sf.v1 = result.seq; sf.env = result.new_env;
                            K.push_back(std::move(sf));
                    ...
                    if (ftag == FRAME_REINSTALL_HANDLER) {
                        ctx->handler_stack.push_back(frame.v1); continue;
                    }
                    ...
                    if (ftag == FRAME_MAKE_PARAMETER) {
                        V = make_parameter(V, frame.v1); continue;
                    }
                        ...
                        if (args.empty()) {
                            ...
                            if (ctx->_instrumented && ctx->tracer && ctx->tracer->_active) {
                                ...
                                if (!trc_name.empty() && trc->_fns_to_trace.count(trc_name)) {
                                    ...
                                    if (trc_printed) trc->_depth = trc_depth + 1;
                                }
                            }
                            if (is_primitive(V)) {
                                std::vector<Value> ea;
                            ...
                            if (trc_printed) {
                                Frame tf; tf.tag = FRAME_TRACE_EXIT;
                                tf.str1 = trc_name; tf.depth = trc_depth;
                                K.push_back(std::move(tf));
                            }
                            E = r.new_env;
                            C = car(r.body);
                            if (is_cons(cdr(r.body))) {
                                Frame sf; sf.tag = FRAME_SEQ;
                                sf.v1 = cdr(r.body); sf.env = r.new_env;
                                K.push_back(std::move(sf));
                            }
                            break;
                        }
                    ...
                    if (ftag == FRAME_CALL) {
                        ...
                        if (is_cwv_prim(fn_value)) {
                            ...
                            Frame cf; cf.tag = FRAME_CWV_CONSUMER;
                            cf.v1 = consumer; cf.v2 = app_node;
                            K.push_back(std::move(cf));
                            fn_value = producer;
                            new_collected = {};
                        }
                        ...
                        if (is_force_prim(fn_value)) {
                            ...
                            if (as_promise_is_done(p)) { V = as_promise_payload(p); continue; }
                            Frame ff; ff.tag = FRAME_FORCE_RESULT; ff.v1 = p;
                        ...
                        if (is_make_param_prim(fn_value)) {
                            ...
                            if (n == 1) { V = make_parameter(new_collected[0], NIL_VALUE); continue; }
                            ...
                            Frame mf; mf.tag = FRAME_MAKE_PARAMETER; mf.v1 = converter;
                        ...
                        if (is_guard_eval_prim(fn_value)) {
                            ...
                            // Skip past FRAME_SHADOW_POP frames to find the real top.
                            {
                                ...
                                while (_gi >= 0 && K[_gi].tag == FRAME_SHADOW_POP) --_gi;
                        ...
                        if (is_raise_cont_prim(fn_value)) {
                            ...
                            ctx->handler_stack.pop_back();
                            Frame rf; rf.tag = FRAME_REINSTALL_HANDLER; rf.v1 = handler;
                        ...
                        if (is_with_params_prim(fn_value)) {
                            ...
                            // Skip past FRAME_SHADOW_POP frames to find the real top.
                            {
                                ...
                                while (_pi >= 0 && K[_pi].tag == FRAME_SHADOW_POP) --_pi;
                            ...
                            WindFrame wf; wf.before = inst; wf.after = rest;
                            ctx->wind_stack.push_back(wf);
                            Frame df; df.tag = FRAME_DYNAMIC_WIND_AFTER; df.v1 = rest;
                        ...
                        if (is_dynamic_wind_prim(fn_value)) {
                            ...
                            ctx->wind_stack.push_back(wf);
                            Frame df; df.tag = FRAME_DYNAMIC_WIND_AFTER; df.v1 = after;
                            K.push_back(std::move(df));
                            fn_value = thunk;
                            new_collected = {};
                        }
                        ...
                        if (is_record_mutator(fn_value)) {
                            ...
                            if (!is_record(rec) || as_record_type(rec) != rt) {
                                ...
                                throw SchemeTypeError(
                                    ...
                                    ": first argument is not a " + rt->name, src);
                            }
                    ...
                    if (ftag == FRAME_SEQ) {
                        ...
                        if (is_cons(cdr(remaining))) {
                            Frame sf; sf.tag = FRAME_SEQ;
                            sf.v1 = cdr(remaining); sf.env = frame.env;
                            K.push_back(std::move(sf));
                        }
                        break;
                    }
                    ...
                    // ── FRAME_WHEN ────────────────────────────────────────────
                    if (ftag == FRAME_WHEN) {
                        Value body = frame.v1; E = frame.env;
                        if (is_false_val(V)) { V = VOID_VALUE; continue; }
                        C = car(body);
                        if (is_cons(cdr(body))) {
                            Frame sf; sf.tag = FRAME_SEQ; sf.v1 = cdr(body); sf.env = frame.env;
                            K.push_back(std::move(sf));
                        }
                        break;
                    }
                    ...
                    // ── FRAME_AND ─────────────────────────────────────────────
                    if (ftag == FRAME_AND) {
                        Value remaining = frame.v1; E = frame.env;
                        if (is_false_val(V)) continue;
                        if (is_nil(remaining)) continue;
                        if (is_cons(cdr(remaining))) {
                            Frame sf; sf.tag = FRAME_AND; sf.v1 = cdr(remaining); sf.env = frame.env;
                            K.push_back(std::move(sf));
                        }
                        C = car(remaining); break;
                    }
                    ...
                    if (ftag == FRAME_OR) {
                        ...
                        if (is_nil(remaining)) continue;
                        if (is_cons(cdr(remaining))) {
                            Frame sf; sf.tag = FRAME_OR; sf.v1 = cdr(remaining); sf.env = frame.env;
                            K.push_back(std::move(sf));
                        }
                        C = car(remaining); break;
                    }
                    ...
                    if (ftag == FRAME_COND) {
                        ...
                        Environment* saved_env = frame.env;
                        if (is_false_val(V)) {
                            if (is_nil(remaining)) { V = VOID_VALUE; continue; }
                            Value nxt  = car(remaining);
                            Value rest = cdr(remaining);
                            CondClause kind = classify_cond_clause(nxt, saved_env);
                            E = saved_env;
                            if (kind.kind == CondClause::Else) {
                                Value body = kind.body_cons; C = car(body);
                                if (is_cons(cdr(body))) {
                                    Frame sf; sf.tag = FRAME_SEQ; sf.v1 = cdr(body); sf.env = saved_env;
                                    K.push_back(std::move(sf));
                                }
                                break;
                            }
                            Frame cf; cf.tag = FRAME_COND; cf.v1 = nxt; cf.v2 = rest; cf.env = saved_env;
                            K.push_back(std::move(cf));
                            C = kind.test; break;
                        }
                        // Test truthy
                        CondClause kind = classify_cond_clause(current, saved_env);
                        if (kind.kind == CondClause::TestOnly) continue;
                        if (kind.kind == CondClause::Body) {
                            Value body = kind.body_cons; E = saved_env; C = car(body);
                            if (is_cons(cdr(body))) {
                                Frame sf; sf.tag = FRAME_SEQ; sf.v1 = cdr(body); sf.env = saved_env;
                                K.push_back(std::move(sf));
                            }
                        ...
                        // Arrow
                        Frame af; af.tag = FRAME_COND_ARROW; af.v1 = V; af.env = saved_env;
                        K.push_back(std::move(af));
                        C = kind.proc_expr; E = saved_env; break;
                    }
                    ...
                    if (ftag == FRAME_COND_ARROW) {
                        ...
                        if (is_continuation(V)) {
                            ...
                            restore_handler_stack(ctx, as_continuation_handlers(V));
                            K = *static_cast<const KStack*>(as_continuation_frames(V));
                            V = continuation_value({ test_value }); continue;
                        }
                        auto pv = apply_parameter_if(V, 1, nullptr);
                        if (pv.has_value()) { V = *pv; continue; }
                        if (is_primitive(V)) {
                            std::vector<Value> a = { test_value };
                            V = as_primitive_fn(V)(ctx, saved_env, a, nullptr); continue;
                        }
                        BetaResult r = apply_value(V, { test_value }, nullptr);
                        E = r.new_env; C = car(r.body);
                        if (is_cons(cdr(r.body))) {
                            Frame sf; sf.tag = FRAME_SEQ; sf.v1 = cdr(r.body); sf.env = r.new_env;
                            K.push_back(std::move(sf));
                        }
                        break;
                    }
                    ...
                    if (ftag == FRAME_CASE_ARROW) {
                        ...
                        if (is_continuation(V)) {
                            ...
                            restore_handler_stack(ctx, as_continuation_handlers(V));
                            K = *static_cast<const KStack*>(as_continuation_frames(V));
                            V = continuation_value({ key_value }); continue;
                        }
                        auto pv = apply_parameter_if(V, 1, nullptr);
                        if (pv.has_value()) { V = *pv; continue; }
                        if (is_primitive(V)) {
                    ...
                    if (ftag == FRAME_CASE) {
                        ...
                        if (is_aux_keyword(head, "else", saved_env)) {
                            Value body = cdr(current_clause); E = saved_env;
                            if (is_cons(body) && is_symbol(car(body)) && as_symbol(car(body)) == "=>") {
                                Frame af; af.tag = FRAME_CASE_ARROW; af.v1 = V; af.env = saved_env;
                                K.push_back(std::move(af));
                                C = car(cdr(body));
                            } else {
                                C = car(body);
                        ...
                        Value cur2 = head;
                        while (is_cons(cur2)) {
                            if (eqv_atom(V, car(cur2))) { matched = true; break; }
                            cur2 = cdr(cur2);
                        }
                        if (matched) {
                            Value body = cdr(current_clause); E = saved_env;
                        ...
                        if (is_nil(remaining)) { V = VOID_VALUE; continue; }
                        Frame cf; cf.tag = FRAME_CASE;
                        cf.v1 = car(remaining); cf.v2 = cdr(remaining); cf.env = saved_env;
                        K.push_back(std::move(cf));
                        continue;
                    }
                    ...
                    if (ftag == FRAME_LET) {
                        ...
                        if (remaining.empty()) {
                            ...
                            E = new_env; C = car(body);
                            if (is_cons(cdr(body))) {
                                Frame sf; sf.tag = FRAME_SEQ; sf.v1 = cdr(body); sf.env = new_env;
                                K.push_back(std::move(sf));
                            }
                            break;
                        }
                        std::vector<Value> new_remaining(remaining.begin()+1, remaining.end());
                        Frame nf; nf.tag = FRAME_LET;
                        nf.ids   = names_ids;
                        nf.list1 = std::move(new_collected);
                        nf.list2 = std::move(new_remaining);
                        nf.v1    = body;
                        nf.env   = saved_env;
                        K.push_back(std::move(nf));
                        C = remaining[0]; E = saved_env; break;
                    }
                    ...
                    if (ftag == FRAME_LET_STAR) {
                        ...
                        Frame nf; nf.tag = FRAME_LET_STAR;
                        nf.uid   = next_pair.first;
                        nf.pairs = std::move(new_remaining);
                        nf.v1    = body;
                        nf.env   = new_env;
                        K.push_back(std::move(nf));
                        C = next_pair.second; E = new_env; break;
                    }
                    ...
                    if (ftag == FRAME_LETREC) {
                        ...
                        auto next_pair = remaining[0];
                        std::vector<std::pair<uint32_t,Value>> new_remaining(remaining.begin()+1, remaining.end());
                        Frame nf; nf.tag = FRAME_LETREC;
                        nf.uid   = next_pair.first;
                        nf.pairs = std::move(new_remaining);
                        nf.v1    = body;
                        nf.env   = saved_env;
                        K.push_back(std::move(nf));
                        C = next_pair.second; E = saved_env; break;
                    }
            ...
            if (!dispatch_exc(make_read_error_object(e.msg, {}), false, false)) throw;
        } catch (PositionedSchemeError& e) {
            if (!dispatch_exc(make_error_object(e.msg, {}), false, false)) throw;
        }
    } // end A
}
...
Value cek_eval(const Value& expr, Environment* env, Context* ctx)
{
    ...
    try {
        ...
        cleanup(true, &e); throw;
    } catch (SchemeSyntaxError& e) {
        cleanup(true, &e); throw;
    } catch (PositionedSchemeError& e) {
        cleanup(true, &e); throw;
    } catch (...) {
        cleanup(false, nullptr); throw;
    }
}
...
void set_global_env(Environment* env) {
    ...
    if (!s_registered) { gc_env_root_push(&g_global_env); s_registered = true; }
    g_global_env = env;
}
Environment* get_global_env()         { return g_global_env; }
