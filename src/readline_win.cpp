...
enum KeyCode {
    ...
    KEY_HOME, KEY_END,
    KEY_CTRL_C, KEY_CTRL_R, KEY_EOF, KEY_ESCAPE,
    KEY_PRINTABLE, KEY_UNKNOWN,
};

struct Key { KeyCode code; wchar_t ch; };

// Second-byte map for \xe0-prefixed extended key sequences.
static Key decode_extended(wchar_t ch2) {
    switch (ch2) {
    case L'\x48': return { KEY_UP,     0 };
    case L'\x50': return { KEY_DOWN,   0 };
    case L'\x4b': return { KEY_LEFT,   0 };
    case L'\x4d': return { KEY_RIGHT,  0 };
    case L'\x47': return { KEY_HOME,   0 };
    case L'\x4f': return { KEY_END,    0 };
    case L'\x53': return { KEY_DELETE, 0 };
    default:      return { KEY_UNKNOWN, 0 };
    }
}

static Key read_key() {
    wchar_t ch = _getwch();
    if (ch == L'\x00' || ch == L'\xe0') return decode_extended(_getwch());
    if (ch == L'\r')                     return { KEY_ENTER,     0 };
    if (ch == L'\x08')                   return { KEY_BACKSPACE,  0 };
    if (ch == L'\x03')                   return { KEY_CTRL_C,     0 };
    if (ch == L'\x12')                   return { KEY_CTRL_R,     0 };
    if (ch == L'\x1b')                   return { KEY_ESCAPE,     0 };
    if (ch == L'\x04' || ch == L'\x1a')  return { KEY_EOF,        0 };
    if (iswprint(ch))                    return { KEY_PRINTABLE,  ch };
...
static std::vector<std::string> split_wbuf(const std::vector<wchar_t>& buf) {
    ...
    for (wchar_t c : buf) {
        if (c == L'\n') { result.push_back(wvec_to_utf8(cur)); cur.clear(); }
...
static void do_redraw(const std::vector<wchar_t>& buf, int cursor,
    ...
    if (leftover > 0) {
        for (int k = 0; k < leftover; ++k) std::cout << "\n\r\033[K";
    ...
    if (lines_up > 0) std::cout << "\033[" << lines_up << "A";
...
static void do_move_end_newline(const std::vector<wchar_t>& buf, int cursor) {
    ...
    if (lines_down > 0) std::cout << "\033[" << lines_down << "B";
    std::cout << '\n';
    std::cout.flush();
}
...
std::string readline_win_input_line(const std::string& prompt,
    ...
    while (true) {
        Key key;
        if (has_pending) { key = pending_key; has_pending = false; }
        else             key = read_key();
        ...
        case KEY_LEFT:
            if (cursor > 0) { --cursor; do_redraw(buf, cursor, prompt, continuation_prompt, prev_extra); }
            break;

        case KEY_RIGHT:
            if (cursor < (int)buf.size()) { ++cursor; do_redraw(buf, cursor, prompt, continuation_prompt, prev_extra); }
            break;
        ...
        case KEY_CTRL_R: {
            ...
            auto search_draw = [&]() {
                ...
                if (prev_extra > 0) { std::cout << "\033[" << prev_extra << "A"; prev_extra = 0; }
            ...
            while (true) {
                ...
                if (skey.code == KEY_CTRL_R) {
                    if (!search_str.empty()) {
                        if      (found_idx > 0)  do_hist_search(found_idx - 1);
                        else if (found_idx == -1) do_hist_search((int)s_history.size() - 1);
                    ...
                    if (!search_str.empty()) {
                        search_str.pop_back();
                        if (!search_str.empty()) do_hist_search((int)s_history.size() - 1);
                        else { found_idx = -1; found_buf.clear(); }
                    }
                    search_draw();
...
void readline_win_add_history(const std::string& entry) {
    ...
    if (!s_history.empty() && s_history.back() == entry) return;
    s_history.push_back(entry);
    if ((int)s_history.size() > s_history_max)
        s_history.erase(s_history.begin());
}

void readline_win_set_history_length(int n) { s_history_max = n; }
...
void readline_win_read_history_file(const std::string& path) {
    ...
    while (std::getline(f, raw)) {
        ...
        if (!entry.empty()) s_history.push_back(entry);
    }
}
...
std::vector<std::string> readline_win_get_history()                        { return s_history; }
void readline_win_set_history(const std::vector<std::string>& entries)     { s_history = entries; }
