...
PositionedSchemeError::~PositionedSchemeError() { delete src; }
...
Value Environment::bind_id(uint32_t sid, Value value) {
    if (_is_immutable)
        throw SchemeTypeError("cannot define '" + display_name(sid)
    ...
    while (scope) {
        ...
        if (it != scope->_bindings.end()) {
            if (scope->_is_immutable)
                throw SchemeTypeError("set! on '" + display_name(sid)
                                      + "' in a frozen environment");
            it->second = value;
